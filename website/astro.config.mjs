...
const metroGrammar = JSON.parse(
  readFileSync(new URL('./src/grammars/metro.tmLanguage.json', import.meta.url), 'utf8'),
);
...
function buildReleasesSidebar() {
  ...
  return [
    ...
    ...[...groups.entries()].map(([label, vers], index) => ({
      ...
      collapsed: index !== 0,
      items: vers.map((version) => ({ label: `v${version}`, slug: `releases/${version}` })),
    })),
  ];
}
  ...
  fonts: [
    {
      ...
      options: {
        variants: [{ weight: 600, style: 'normal', src: ['./src/fonts/Degular-Semibold.woff2'] }],
      },
    },
  ],
  integrations: [
    ...
    starlight({
      ...
      social: [{ icon: 'github', label: 'GitHub', href: 'https://github.com/pinin4fjords/nf-metro' }],
      editLink: {
      ...
      sidebar: [
        {
          ...
          items: [
            ...
            { label: 'Playground (beta)', link: `${base}playground/`, attrs: { target: '_self' } },
          ],
        },
        {
