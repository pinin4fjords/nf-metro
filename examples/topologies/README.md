...
| `packed_cell_cellmate_bypass_own_descent.mmd` | packed cell whose far member and a following section are both entered over a cell-mate, with no sibling hop able to carry a shared descent, so each draws its own (issue #1844)                                                 |
