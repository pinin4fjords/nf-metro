...
For `%%metro line:` specifically: a second declaration of an id already
declared is dropped, so the first one wins; a declaration missing its id, name
or colour is dropped whole, which leaves the id undeclared and makes any edge
annotated with it a hard error; and a colour that is neither a hex value, a CSS
colour name, nor a functional notation like `rgb(...)` is reported but kept, so
a colour syntax nf-metro does not model reaches the SVG untouched.
