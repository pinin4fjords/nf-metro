...
```bash frame="terminal"
# Grey out just the pseudo-alignment line for this render
nf-metro render pipeline.mmd -o run.svg --inactive-lines salmon

# Show every line at full colour, ignoring the map's own inactive defaults
...
| `%%metro style: <name>`                                                    | Brand theme: `nfcore` (default; `dark` is an accepted alias), `seqera`, or `light` (the transparent embed theme). The brand names also take a `-light`/`-dark` suffix to pin a mode. Selects the render theme unless `--theme` is passed.                                                                                                                                                              |
| `%%metro mode: <light\|dark>`                                              | Display mode, an axis independent of the brand. An SVG carries both palettes via CSS `light-dark()` and adapts to the viewer's `color-scheme`, so this only needs setting to bake a concrete palette (a PNG export, say). CLI equivalent `--mode`.                                                                                                                                                     |
| `%%metro line: <id> \| <name> \| <color> [\| <style> [\| inactive]]`       | Define a metro line. Optional style: `solid` (default), `dashed`, or `dotted`. Optional fifth field `inactive` greys the line out by default (override per-render with `--inactive-lines`)                                                                                                                                                                                                             |
...
| `%%metro marker: <station> \| <shape>, <fill>`                             | Override one station's marker so its shape and fill encode a tool attribute (see above). `shape` defaults to `circle` and `fill` to `solid`.                                                                                                                                                                                                                                                           |
| `%%metro marker_legend: <shape>, <fill> \| <caption>`                      | Add a marker key below the line legend, one row per shape/fill combination (see above).                                                                                                                                                                                                                                                                                                                |
...
- A second declaration of an already-declared id is dropped, so the first one
  wins.
- A declaration missing its id, name or colour is dropped whole. Any edge
  annotated with a line the map only tried to declare is then a hard error,
  rather than a map that renders with no lines.
- A colour that is neither a hex value, a CSS colour name nor a functional
  notation like `rgb(...)` is reported but kept, so a colour syntax nf-metro
  does not model reaches the SVG as written.
