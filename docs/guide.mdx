...
**Logos.** `%%metro logo: <path>` bundles one image into the legend, and nf-metro uses that image in every display mode. `%%metro logo: <light-path> | <dark-path>` supplies a pair instead.
...
Baking a concrete palette with `--no-chrome-css`, which a PNG export needs, drops the masks instead and keeps only the asset for the resolved mode. `--mode dark` therefore bakes the dark logo and `--mode light` the light one.
...
Both paths resolve relative to the `.mmd` file, and either one failing to resolve is an error. `--logo` sets the single-path form only, does not replace a pair declared in the file, and takes a path from the working directory rather than from the `.mmd`.
...
The following table is the authority on the exceptions. `style:` is spelled `--theme` on the command line. `manifest:` has no flag you should reach for, because it writes a graph field named `embed_manifest` and its flag is an internal escape hatch kept out of `--help`.
...
They select the output target, a diagnostic overlay, or how the SVG is packaged for a host, rather than describing the diagram. `--inactive-lines` is the per-render counterpart to the `inactive` field on `%%metro line:`, and replaces that set outright.
