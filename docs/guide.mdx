...
| `%%metro track_gap: <pixels>`                                              | Visual gap between adjacent line strokes in a bundle (0–3 px, edge to edge). `0` collapses all lines onto a single shared track; default is 1 px (the built-in 4 px centre-to-centre minus the 3 px stroke). CLI equivalent `--track-gap`.                                                                                                                                                            |
...
**Track gap.** `%%metro track_gap: <px>` sets the visual gap between adjacent line strokes in a bundle - the empty space between their edges, not their centres. The default is 1 px (the built-in 4 px centre-to-centre minus the 3 px nfcore stroke). Set it to `0` to collapse all lines onto a single shared track, which suits maps where the distinction between co-running lines is already conveyed by section structure rather than visible separation:
