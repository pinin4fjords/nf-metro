...
3. **Breaks cycles.** nf-metro's layout engine requires a DAG, so a process
   graph that loops cannot be drawn edge for edge. The converter removes a
   deterministic set of back edges, including any process that reaches itself.
   It reports every removed connection on stderr and lists them in a `%%`
   comment block at the end of the output, so a loop can be redrawn by hand
...
4. **Maps subworkflows to sections.** Nextflow subworkflows become nf-metro `subgraph` sections. Processes that belong to no subworkflow are grouped into generated sections.
