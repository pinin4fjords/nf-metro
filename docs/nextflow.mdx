...
Direct conversion works well for pipelines with a few subworkflows. For complex pipelines such as those in nf-core, it is unlikely to produce a good diagram, and you need to hand-write or heavily edit the `.mmd` file. This is an active area of development.
:::
...
3. **Breaks cycles.** The nf-metro layout engine requires a DAG. A process
   graph that loops therefore cannot be drawn edge for edge. The converter
   removes a deterministic set of back edges, including any process that
   reaches itself. It reports every removed connection on stderr and lists
   them in a `%%` comment block at the end of the output. You can then redraw
   a loop by hand rather than lose it silently.
