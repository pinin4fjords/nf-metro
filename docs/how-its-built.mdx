...
The codebase has four complementary validation layers. They are complementary, not redundant: each catches bugs the others cannot see. These are described in detail in the [Testing docs](../dev/testing/#the-four-validation-layers); the short version:
...
The full tier taxonomy and rationale is in [`docs/dev/guard_tiers.md`](../dev/guard_tiers/).

## Routing dispatch

The routing module uses a [first-match dispatcher](https://github.com/pinin4fjords/nf-metro/blob/main/src/nf_metro/layout/routing/core.py) over a table of handler families. Each handler covers a specific combination of section orientation, entry/exit direction, and flow type. The dispatch table and the full handler inventory are documented in [`docs/dev/inter_section_dispatch.mdx`](../dev/inter_section_dispatch/) and [`docs/dev/routing_gate_coverage.md`](../dev/routing_gate_coverage/).
...
- [`architecture.mdx`](../dev/architecture/) - system overview and data flow
- [`layout_pipeline.mdx`](../dev/layout_pipeline/) - the phase sequence with entry/exit contracts
- [`routing.mdx`](../dev/routing/) - edge routing mechanics and the centreline/offset model
- [`inter_section_dispatch.mdx`](../dev/inter_section_dispatch/) - the dispatch table for inter-section routes
- [`routing_gate_coverage.md`](../dev/routing_gate_coverage/) - which handler arms are covered by corpus tests
- [`parser.mdx`](../dev/parser/) - the Lark grammar and post-parse rewrites
- [`testing.md`](../dev/testing/) - test structure and validation layers in detail
