...
| 1202 | `go is not None` | `->L1208` |  |
| 1203 | `and gt is not None` | `->L1208` |  |
| 1240 | `if len(pts) < 2:` | `->L1241` |  |
| 1256 | `if abs(y2 - y1) > COORD_TOLERANCE or abs(x2 - x1) <= COORD_TOLERANCE:` | `->L1257` |  |
| 1333 | `if slot is None or trunk is None or trunk[0] != k:` | `->L1334` | **needs-review** -- _reconcile_moved_trunk_slot's precondition guard: the re-banded leg is not the route's first interior horizontal run, or the route declared no trunk slot. The docstring states this as a real filter ('only the trunk the slot describes can retarget it'), and _bundle_divergent_distinct_traverses hands it every distinct-line traverse leg, so a non-trunk leg or a slotless route reaches it; all 3 corpus fixtures happen to pass. Reachable; wants a fixture. |
| 1344 | `if any(abs(channel.x - ref_x) > COORD_TOLERANCE for channel in planned[1:]):` | `->L1345` |  |
| 1345 | `if ctx.validate_final_route_frames:` | `->L1346`, `->L1347` |  |
| 1391 | `if channel is None:` | `->L1392` |  |
| 1464 | `if planned and max(planned) - min(planned) > COORD_TOLERANCE:` | `->L1465` |  |
| 1470 | `if ref is not None:` | `->L1471` |  |
| 1537 | `for group in _merge_feeder_groups(routes, ctx):` | `->L1538` |  |
| 1541 | `if movable_channels:` | `->L1537`, `->L1542` |  |
| 1579 | `if not downs or not ups:` | `->L1581` |  |
| 1582 | `for descent in downs:` | `->L-1548`, `->L1583` |  |
| 1584 | `if col in moved:` | `->L1585`, `->L1586` |  |
| 1587 | `for up in ups:` | `->L1588`, `->L1597` |  |
| 1588 | `if up.route.line_id != descent.route.line_id:` | `->L1589`, `->L1590` | **defensive** -- _clear_merge_trunk_opposite_arm skips an up-arm carrying a different line from the down-trunk, since only a same-line opposite arm folds the line back over itself. The corpus's one fork sends a single line both ways, so the different-line skip arm stays un-exercised. Defensive against a fork whose opposite arms carry distinct lines. |
| 1590 | `if abs(up.x - descent.x) > radius:` | `->L1591`, `->L1592` |  |
| 1593 | `if overlap <= COORD_TOLERANCE:` | `->L1594`, `->L1595` | **defensive** -- _clear_merge_trunk_opposite_arm skips an opposite arm within a curve radius of the down-trunk but not overlapping it in Y, since without a shared Y span there is no fold-back to clear. The corpus's qualifying fork overlaps in Y, so the no-overlap skip arm stays un-exercised. Defensive against a near-column opposite arm that clears the trunk vertically. |
| 1597 | `if target_x is None:` | `->L1598`, `->L1599` |  |
| 1600 | `for ch in downs:` | `->L1582`, `->L1601` |  |
| 1601 | `if abs(ch.x - descent.x) <= COORD_TOLERANCE:` | `->L1600`, `->L1602` | **defensive** -- _clear_merge_trunk_opposite_arm moves every down-leg sharing the folding descent's column and skips those on other columns. The corpus's one folding descent carries all its down-legs on a single shared column, so the other-column skip arm stays un-exercised. Defensive against a fork with down-legs spread over several columns. |
| 1621 | `if k == desc.idx + 1:` | `->L1620` | **defensive** -- In _fanout_traverse_leg: the corpus fan-out routes carry the traverse as their only interior horizontal trunk, so the loop matches on the first trunk and the non-match arm is never taken. Defensive. |
| 1770 | `if lower > upper + COORD_TOLERANCE_FINE:` | `->L1771` |  |
| 1781 | `if anchored and abs(target - reference) > COORD_TOLERANCE_FINE:` | `->L1782` | **defensive** -- In _coincide_same_line_fanout_traverses: a fan-owned traverse is snapped onto its convergence-plan-owned sibling's band, but the movable member's own reserved claim band may forbid that Y. The corpus fan-owned traverses carry no claim band that conflicts with the anchor, so the target always lands on the reference and the skip arm is never taken. Defensive against a movable member whose reservation excludes the anchor's band. |
| 1789 | `if any(` | `->L1799` |  |
| 1819 | `abs(p0[1] - p1[1]) > COORD_TOLERANCE` | `->L1823` |  |
| 1820 | `or abs(p1[0] - p2[0]) > COORD_TOLERANCE` | `->L1823` |  |
| 1821 | `or abs(p2[1] - p3[1]) > COORD_TOLERANCE` | `->L1823` |  |
| 1840 | `if source_col is None:` | `->L1841` |  |
| 1856 | `if route_system_owns_segment_boundary(route, opening.idx):` | `->L1857` |  |
| 1871 | `if not sibling_xs:` | `->L1872` |  |
| 1874 | `if new_x <= max(sibling_xs) + ctx.curve_radius + COORD_TOLERANCE:` | `->L1875` |  |
| 1877 | `if _section_intrudes(` | `->L1884` |  |
| 1921 | `if len(turns) != 1:` | `->L1922` | **defensive** -- In _bundle_divergent_distinct_traverses: every fan in the corpus turns its traverses the same way, so the mixed-direction skip is never taken. Defensive. |
| 1947 | `if any(` | `->L1951` |  |
...
| 2046 | `if (` | `->L2054` | **defensive** -- The coincident-corner repair rejects a candidate radius only for synthetic or directly supplied unowned geometry. Production member-owned corners already fit their frozen reference; focused concentric-corner tests cover the repair path. |
| 2057 | `if not reference_fits(reference):` | `->L2058` | **defensive** -- Production coincident members publish a clamp-safe shared reference. The bisection arm repairs synthetic or future unowned geometry and is covered by focused normalization unit tests. |
| 2068 | `if not reference_fits(lower):` | `->L2069`, `->L2070` | **defensive** -- The lower bound is constructed from each member's minimum resolvable concentric radius, so at least that bound fits valid route geometry. Failure denotes contradictory synthetic corner inputs. |
| 2071 | `for _step in range(64):` | `->L2072`, `->L2077` | **defensive** -- No top-level fixture can enter this defensive bisection honestly: production short leads at shared corners are frozen member-plan geometry, so corner_is_owned skips them, while unowned production legs have enough runway to share the preferred radius directly. The direct synthetic test_unify_uses_widest_radius_all_shared_legs_can_resolve exercises both loop completion and the clamp-safe correction. |
| 2073 | `if reference_fits(midpoint):` | `->L2074`, `->L2076` | **defensive** -- Both bisection decisions belong to the synthetic clamp-repair path. Production member-owned references bypass the search; unit tests exercise convergence on the widest shared radius. |
