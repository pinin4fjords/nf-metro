...
**1957/2840** gates fully exercised (both/all arms hit by some fixture); **883** gates have at least one un-exercised arm.
...
| 1073 | `if entry_geometry.seam.turn_direction is None:` | `->L1074` |  |
| 1157 | `if abs(turn_delta) <= COORD_TOLERANCE:` | `->L1158` |  |
| 1185 | `if abs(turn_delta) <= COORD_TOLERANCE:` | `->L1186` | **defensive** -- _plain_l_shape_turn_requirement's flat-hop guard, which both its callers exclude: _standard_l_shape_turn_requirement returns its own missing-source-turn for a flat delta before delegating, and the cell-mate gap drop only classifies when the target section sits one grid row below the source (_bypass_route_kind requires tgt_row == src_row + 1), which puts the two ports on different rows. The duplicated guard therefore never fires. |
| 1314 | `if kind is _BypassRoute.LEFT_ENTRY_FAMILY:` | `->L1315` |  |
| 1318 | `if kind is _BypassRoute.RIGHT_ENTRY_CROSS_ROW:` | `->L1319` |  |
| 1320 | `if kind is _BypassRoute.L_SHAPE:` | `->L1321` |  |
| 1327 | `if kind is _BypassRoute.LEFT_EXIT_AROUND_BELOW:` | `->L1328` |  |
| 1460 | `if primary_delta * run_direction.sign < 2 * MIN_STRAIGHT_EDGE + diagonal:` | `->L1461` |  |
| 1487 | `if direction_axis(transition.run_direction) is DemandAxis.X:` | `->L1491` |  |
| 1542 | `if len(candidates) != 1:` | `->L1543` |  |
| 1549 | `if direction_axis(run_direction) is DemandAxis.Y:` | `->L1550` |  |
| 1577 | `if transition is None:` | `->L1578` |  |
| 1614 | `target_crossing is None` | `->L1617` |  |
| 1615 | `or abs(source_crossing - target_crossing) > COORD_TOLERANCE` | `->L1617` |  |
| 1639 | `target_crossing is None` | `->L1642` |  |
| 1640 | `or abs(geometry.cross_lo - target_crossing) > COORD_TOLERANCE` | `->L1642` |  |
| 1652 | `if direction_axis(run_direction) is DemandAxis.Y:` | `->L1653` |  |
| 1678 | `if transition is None:` | `->L1679` |  |
| 1712 | `if direction_axis(run_direction) is DemandAxis.Y:` | `->L1713` |  |
| 1738 | `if transition is None:` | `->L1740` |  |
| 1771 | `if len(entry_group_ids) != 1:` | `->L1772` |  |
| 1778 | `if family_id is None:` | `->L1779` |  |
| 1807 | `requirement.run_direction is not None` | `->L1820` |  |
| 1808 | `and requirement.turn_direction is not None` | `->L1820` |  |
| 1809 | `and requirement.launch_coordinate is not None` | `->L1820` |  |
| 1810 | `and requirement.minimum_runway is not None` | `->L1820` |  |
| 1829 | `and requirement.turn_direction is not None` | `->L1836` |  |
| 1841 | `and family_id is not RouteFamilyId.MERGE_ENTRY` | `->L1844` |  |
| 1869 | `if any(len(runs) > 1 for runs in straight_runs_by_lane.values()):` | `->L1870` | **defensive** -- The opposed-source-run recogniser: a lane whose turn-less members disagree on the direction they leave the shared source. Every member of one exit group leaves through one exit port, whose side fixes the outward ray, and a collinear target on the other side of that port is a backward hop the engine rejects before routing. The arm names a classification that contradicts itself and declines the plan; no exit group produces it. |
| 2021 | `if any(` | `->L2032` |  |
| 2046 | `if membership is not None and membership.axis is not None:` | `->L2047` |  |
| 2124 | `if not _lane_arms_read_as_one_stroke(` | `->L2127` | **defensive** -- The reject protects a planner seed set where an unpinned cohort lands within one curve radius of a same-rank axis already claimed by a pinned cohort. Production topology planning pins or separates those cohorts before this check. test_free_lane_arm_overlapping_a_pinned_corner_uses_legacy constructs the incomplete seed set directly and covers the fallback reason. |
| 2241 | `if reason is not None:` | `->L2242` |  |
| 2344 | `if not outbound_edges or missing_connectors:` | `->L2345` |  |
| 2446 | `if final_classification.legacy_reason is not None:` | `->L2447` |  |
| 2448 | `elif tuple(seed.family_id for seed in final_classification.seeds) != tuple(` | `->L2451` |  |
| 2713 | `if incumbent.id == plan.id:` | `->L2715` |  |
| 2739 | `if prior is None:` | `->L2740` |  |
| 2783 | `if owners and plan.id not in owners:` | `->L2784` |  |
| 2784 | `for owner in owners:` | `->L2785`, `->L2786` |  |
| 2799 | `and plan.id in conflicting_plan_ids` | `->L2801` |  |
| 2817 | `and axis.fixed_anchor_offset is not None` | `->L2806` |  |
| 2818 | `and abs(proposed - axis.fixed_anchor_offset) > COORD_TOLERANCE` | `->L2820` |  |
| 3008 | `if abs(first_offset - second_offset) > COORD_TOLERANCE:` | `->L3010` |  |
| 3010 | `for owner in (first_owner, second_owner):` | `->L3007`, `->L3011` |  |
| 3011 | `if owner is not None:` | `->L3010`, `->L3012` |  |
| 3018 | `if not additions:` | `->L3020` |  |
| 3082 | `if assignment.planned_family_id is RouteFamilyId.BOTTOM_EXIT_JUNCTION:` | `->L3083` |  |
| 3118 | `if route is None:` | `->L3119` |  |
| 3124 | `if assignment.planned_family_id is not RouteFamilyId.TB_BOTTOM_EXIT:` | `->L3125` |  |
| 3139 | `if route is None:` | `->L3140`, `->L3146` |  |
| 3154 | `if geometry.bundle_offsets is None:` | `->L3155` |  |
| 3247 | `if scaffold is None:` | `->L3248` |  |
| 3255 | `if scaffold is None:` | `->L3256` | **defensive** -- The second None check protects the manual or incomplete graph fallback after rebuilding a missing retained scaffold. Parsed render-path graphs publish one canonical scaffold during fan planning, so the fixture corpus never reaches a rebuild that can return None. Direct planner tests retain the fallback for graph construction without topology. |
| 3294 | `if overridden_dispositions:` | `->L3295` |  |
| 3296 | `for plan_id in overridden_dispositions:` | `->L3297`, `->L3308` |  |
| 3352 | `if transition_key in transition_by_edge:` | `->L3353` |  |
| 3376 | `if len(points) >= 3:` | `->L3383` |  |
| 3379 | `segment_direction(before, start) is run_direction` | `->L3383` |  |
| 3380 | `and segment_direction(start, end) is turn_direction` | `->L3383` |  |
| 3424 | `assignment is None` | `->L3429` |  |
| 3425 | `or membership.axis is None` | `->L3429` |  |
| 3426 | `or assignment.run_direction is None` | `->L3429` |  |
| 3427 | `or assignment.turn_direction is None` | `->L3429` |  |
| 3476 | `if ctx.exit_turns is None:` | `->L3477` |  |
| 3487 | `if family_id is not assignment.planned_family_id:` | `->L3488` |  |
| 3500 | `if (` | `->L3509` |  |
| 3518 | `run is None` | `->L3523` |  |
| 3519 | `or turn is None` | `->L3523` |  |
| 3520 | `or assignment.launch_coordinate is None` | `->L3523` |  |
| 3521 | `or assignment.minimum_runway is None` | `->L3523` |  |
| 3527 | `if segment_rank is None:` | `->L3528` |  |
| 3548 | `if planned_corner_offsets is None:` | `->L3549` |  |
| 3615 | `if source_turn_changed and settled is None:` | `->L3616` |  |
| 3645 | `if rank is None:` | `->L3646` |  |
| 3712 | `if current.keys() != snapshot.geometry.keys():` | `->L3713` |  |
| 3727 | `if current[key] != state:` | `->L3728` |  |
| 3760 | `station_id not in graph.stations` | `->L3763` |  |
| 3761 | `or lane.line_id not in graph.station_lines(station_id)` | `->L3763` |  |
| 3769 | `if (station_id, lane.line_id) not in station_offsets or (` | `->L3776` |  |
| 3784 | `if len(transition_routes) != 1:` | `->L3785` |  |
| 3822 | `route.exit_lane_transition_plan_id != str(exit_turn_plan.id)` | `->L3827` |  |
| 3823 | `or route.offset_regime is not OffsetRegime.BAKED` | `->L3827` |  |
| 3824 | `or route.points != expected_route.points` | `->L3827` |  |
| 3825 | `or route.curve_radii != expected_route.curve_radii` | `->L3827` |  |
| 3843 | `if not _fixed_axis_matches_plan(` | `->L3848` |  |
| 3854 | `if any(` | `->L3863` |  |
| 3871 | `if len(member_routes) != 1:` | `->L3872` |  |
| 3879 | `if route.exit_turn_family_id != assignment.planned_family_id.value:` | `->L3880` |  |
| 3886 | `if route.route_system_disposition == "compatibility":` | `->L3887` |  |
| 3889 | `if route.exit_turn_axis_id is not None:` | `->L3890` |  |
| 3898 | `if (` | `->L3912` |  |
| 3920 | `route.exit_turn_axis_id != str(assignment.axis_id)` | `->L3923` |  |
| 3921 | `or route.exit_turn_segment_rank is None` | `->L3923` |  |
| 3934 | `if (` | `->L3956` | **defensive** -- The emission validation that raises when a rendered turn's axis, lead-in runway or direction differs from the assignment the route was bound to. The emitters derive all four from that same assignment, so the raise names an emitter ignoring its plan rather than a topology. |
