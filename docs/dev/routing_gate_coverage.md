...
**2241/3180** gates fully exercised (both/all arms hit by some fixture); **939** gates have at least one un-exercised arm.
...
| 4914 | `if len(points) < 3:` | `->L4915` |  |
| 4966 | `if not section_ids:` | `->L4967` |  |
| 5046 | `if mid_y is None:` | `->L5063` |  |
| 5078 | `channel_y is None` | `->L5108` |  |
| 5080 | `and src_sec is not None` | `->L5108` | **defensive** -- Inter-row exit mid-Y clearance bump (the n>1 multi-line branch). When more than one line shares the exit the source section always resolves, so the `src_sec is None` short-circuit past the clearance bump is never taken. |
| 5081 | `and tgt_sec is not None` | `->L5108` | **defensive** -- Companion to src_sec in the same n>1 mid-Y clearance branch: the target section always resolves there, so the `tgt_sec is None` short-circuit past the bump is never taken. |
| 5110 | `if exit_side is not None:` | `->L5112` |  |
| 5112 | `elif abs(dx) > ctx.curve_radius:` | `->L5113`, `->L5115` |  |
| 5116 | `if src.id in ctx.graph.junctions:` | `->L5117`, `->L5129` | **needs-review** -- Reachable only via a defective render (LR->TB TOP-entry routing); see #720. |
| 5117 | `for je in ctx.graph.edges_to(src.id):` | `->L5118`, `->L5129` |  |
| 5119 | `if js.is_port:` | `->L5117`, `->L5120` |  |
| 5120 | `if abs(js.x - src.x) <= COORD_TOLERANCE:` | `->L5121`, `->L5123` |  |
| 5135 | `if exit_side is not None and not straight_drop:` | `->L5145` |  |
| 5183 | `if not straight_drop:` | `->L5187` | **needs-review** -- A top-entry fan branch fed straight from directly above the junction (straight_drop) keeps its lead-in column at the source X rather than the shared fan pivot; the corpus's fan branches all carry horizontal travel, so the straight-drop arm is unexercised. |
| 5185 | `if lead is Direction.R:` | `->L5187` | **needs-review** -- A top-entry fan branch leading LEFT out of the junction skips the _v1_corner_x right-edge clearance bump; the corpus's fan branches all lead right, so the left-lead arm is unexercised. |
| 5212 | `if straight_drop:` | `->L5213` |  |
| 5289 | `if ctx.exit_turns is None:` | `->L5290` | **needs-review** -- _perp_entry_turn_is_planned answers False for the pre-plan context the exit-turn planner probes emitters with; ctx.exit_turns is bound only for emission. That context does reach these emitters -- the same guard in _seat_bypass_descent takes its None arm on packed_cell_right_exit_left_entry_wrap -- but no corpus probe reaches the TOP/BOTTOM-entry L-shape records this predicate feeds. |
| 5374 | `if src_sec is None or tgt_sec is None or src_sec.grid_row >= tgt_sec.grid_row:` | `->L5377` |  |
| 5544 | `if src_section and src_section.bbox_w > 0:` | `->L5545` |  |
| 5565 | `if tgt_sec is None:` | `->L5566` |  |
| 5636 | `if src_col is None:` | `->L5637` |  |
| 5871 | `if src_sec is not None:` | `->L5902` | **defensive** -- Defensive guard in the fanned entry-wrap source-clearance lift: a fan-out junction always resolves to its source section, so the None arm is unreachable in practice. |
| 5908 | `if tgt_col is not None:` | `->L5913` | **defensive** -- tgt_col from _resolve_section_col on a valid LEFT entry port is always non-None (sections get grid_col>=0 after auto-layout). |
| 5910 | `if shared_vx is not None:` | `->L5913` | **defensive** -- _fan_left_entry_descent_x returns None only when col_left<=0; col_left_edge for the target's own column is always >0. |
| 6036 | `if entry_side is PortSide.LEFT:` | `->L6039` |  |
| 6104 | `if entry_port is None:` | `->L6105` | **defensive** -- _route_around_section_below is always called with a non-None entry_port (both dispatch sites pass a checked station). |
| 6115 | `if trunk_src is None or trunk_src == edge.source:` | `->L6117` | **needs-review** -- Reachable only via a defective render (merge-junction tangle); see #721. |
| 6168 | `if col_left <= 0.0:` | `->L6169` | **defensive** -- col_left_edge's 0.0 default only fires for a column with no sections; the target's own column always holds its section. |
| 6205 | `if entry_port is None:` | `->L6206` | **defensive** -- Every caller of _corridor_is_viable passes a non-None entry_port. |
| 6212 | `if src_row is None or ep_row is None or src_col is None or ep_col is None:` | `->L6213` | **defensive** -- _resolve_section_colrow returns (None,None) only for a station with no section; valid entry ports always have a section with non-negative grid coords. |
| 6299 | `if fan is not None and corridor is not None and corridor.band_y is not None:` | `->L6303` | **needs-review** -- Corridor-feeder shared inter-row band gate: the corpus's corridor feeders belong to fans whose corridor carries a band_y, so the short-circuit past it (no corridor or bypass-only) is unexercised; it falls to the elif fan-is-not-None global-band branch. |
| 6303 | `elif fan is not None:` | `->L6307`, `->L6312` | **needs-review** -- Corridor-feeder fallback for a fan whose junction earned no corridor (its in-column gap below does not fit the bundle); the corpus's fans all earn a corridor, so this global-band branch is unexercised. |
| 6312 | `elif gap_bottom > gap_top:` | `->L6313`, `->L6317` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6322 | `if fan is None and gap_bottom > gap_top:` | `->L6323` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6336 | `if fan is not None and ep_col is not None:` | `->L6338` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6338 | `if vx is None:` | `->L6339` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6440 | `if ep_section and ep_section.bbox_w > 0:` | `->L6443` | **defensive** -- entry_port.section_id is always set and the section has bbox_w>0 after layout; the else arm never fires. |
| 6737 | `for claim in ctx.convergences.prior_channel_claims_for_edge(edge):` | `->L6738` |  |
| 6738 | `if claim.line_id != edge.line_id or claim.owner_source == edge.source:` | `->L6739`, `->L6740` |  |
| 6740 | `if not (corner_x - COORD_TOLERANCE <= claim.x <= gap_right + COORD_TOLERANCE):` | `->L6741`, `->L6742` |  |
| 6742 | `if min(hi, claim.y_hi) - max(lo, claim.y_lo) > COORD_TOLERANCE:` | `->L6737`, `->L6743` |  |
| 6816 | `if _leadout_self_meets_sibling_descent(ctx, edge, corner_x, f.sy, hy, gap_right):` | `->L6817` |  |
| 6948 | `if gap_bottom <= gap_top:` | `->L6949` | **defensive** -- The row-gap enforcer keeps stacked rows separated, so the inter-row band above a target row (row_bottom_edge(tgt_row-1) up to row_top_edge(tgt_row)) has positive height whenever a RIGHT entry is fed from a higher row; the degenerate-band arm guards an abutting-rows layout placement never produces. |
| 6952 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L6953` | **defensive** -- Band-fit test in _right_entry_gap_above_is_clear (#889), reached only when the cross-row RIGHT-entry drop-in is not clear (the exotic blocked-descent case above). The corpus always takes the drop-in, so neither arm of the gap-above fallback is exercised. Defensive. |
| 7107 | `if gap_bottom <= gap_top:` | `->L7108` |  |
| 7109 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L7110` |  |
| 7120 | `if f.v_segment_crosses_other_section(vx, gy, tgt.y):` | `->L7121` |  |
| 7215 | `if source is None or source.bbox_w <= 0:` | `->L7216` |  |
| 7220 | `if source.id not in members:` | `->L7221` |  |
| 7226 | `if other.bbox_x <= source_right + COORD_TOLERANCE:` | `->L7227` |  |
| 7228 | `if cellmate_left is None or other.bbox_x < cellmate_left:` | `->L7222` |  |
| 7230 | `if cellmate_left is None:` | `->L7231` |  |
| 7268 | `if band0_bottom <= band0_top or not _inter_row_band_fits(band0_top, band0_bottom):` | `->L7269` |  |
| 7270 | `if band1_bottom <= band1_top or not _inter_row_band_fits(band1_top, band1_bottom):` | `->L7271` |  |
| 7272 | `if band1_top <= band0_bottom + COORD_TOLERANCE:` | `->L7273` |  |
| 7288 | `if f.v_segment_crosses_other_section(corner_x, band0_y, band1_y, exclude):` | `->L7289` |  |
| 7345 | `if geom is None:` | `->L7346` |  |
| 7348 | `if f.v_segment_crosses_other_section(geom.lead_x, src.y, geom.band0_y):` | `->L7349` |  |
| 7350 | `if f.h_segment_crosses_other_section(geom.lead_x, geom.corner_x, geom.band0_y):` | `->L7351` |  |
| 7352 | `if f.h_segment_crosses_other_section(geom.corner_x, geom.vx, geom.band1_y):` | `->L7353` |  |
