...
**1952/2834** gates fully exercised (both/all arms hit by some fixture); **882** gates have at least one un-exercised arm.
...
81/113 gates fully exercised.

Gates with an un-exercised arm:

| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 198 | `if run_direction is None or turn_direction is None:` | `->L199` |  |
| 200 | `if (run_direction in {Direction.R, Direction.L}) == (` | `->L203` |  |
| 212 | `if not candidate_ranks:` | `->L213` |  |
| 289 | `if ctx.exit_turns is None or not pending_plan_ids:` | `->L290` |  |
| 491 | `if source is None or source is route:` | `->L493` |  |
| 534 | `if route is None:` | `->L535` |  |
| 541 | `if route is None:` | `->L542` |  |
| 574 | `if ctx.exit_turns is not None:` | `->L576` |  |
| 576 | `if ctx.convergences is not None:` | `->L578` |  |
| 752 | `if route.exit_lane_transition_plan_id is not None:` | `->L753` |  |
| 754 | `if route.fan_plan_id is not None or route.fan_route_emitter is not None:` | `->L755` |  |
| 756 | `if convergence_owns_segment_boundary(route, segment_rank):` | `->L757` |  |
...
| 816 | `if bounds.band is not None and not bounds.band.lo <= candidate <= bounds.band.hi:` | `->L817` | **needs-review** -- _candidate_clears_runway's band rejection, unreachable while `if band is not None:::#1` never populates a band on this path. Reachable. |
| 876 | `if abs(channel.x - target) <= COORD_TOLERANCE:` | `->L878` | **needs-review** -- _align_same_line_channels' re-seat arm. Its one fixture (genomeassembly_organellar) still reaches the gate but now finds the channel already on its carrier's claim coordinate, so the reseat is skipped. Bisecting the range shows 0f58d60c -- ranking member channels over a gap's whole population -- is what makes the claim and the channel coincide on arrival, not the bundle-pitch packing. Reachable; the frozen render corpus means no fixture was authored. |
| 901 | `if coordinate is not None:` | `->L856`, `->L902` |  |
| 973 | `if overlap <= MIN_CORRIDOR_Y_OVERLAP:` | `->L974` |  |
| 1067 | `if item.candidate.route.line_id in claim.line_ids and (` | `->L1070` | **needs-review** -- feasible()'s same-line-carrier clause, which forbids a candidate delta that would split a carrier the bundle already shares with a claim. No fixture reaches the feasibility search with a same-line source-compatible claim in range: _align_same_line_channels seats such a member on its claim before the bundle allocation runs. Reachable, wants a fixture. |
| 1070 | `if abs(coordinate - claim.coordinate) > COORD_TOLERANCE:` | `->L1071`, `->L1072` | **needs-review** -- The rejection inside feasible()'s same-line-carrier clause. Unreached for the same reason as its enclosing gate `if item.candidate.route.line_id in claim.line_ids and (:::#1`. Reachable. |
| 1210 | `if radii is not None:` | `->L1215` |  |
| 1211 | `if 0 <= rank - 1 < len(radii):` | `->L1213` |  |
| 1213 | `if rank < len(radii):` | `->L1215` |  |
| 1245 | `elif abs(start[0] - end[0]) <= COORD_TOLERANCE:` | `->L1235` |  |
| 1313 | `if movable_run:` | `->L1314` |  |
| 1358 | `if context is not None:` | `->L1360` | **defensive** -- _convergence_context_route returns None only when ctx.edge_by_key has no entry for the key, and edge_by_key is built from every graph.edge while the key comes from scaffold.edge_order, which is derived from the same edges. Same never-taken lookup as the sibling `if edge is None` guard on the non-convergence path (0 of 141 fixtures). |
| 1361 | `if key in ctx.skip_edges:` | `->L1362` | **candidate-dead** -- Subsumed by the convergence-edge check two lines above. Instrumenting all 348 fixtures: ctx.skip_edges holds exactly one merge-junction->entry-port hop, it is always present in scaffold.edge_order, and it is always also a convergence member edge, so the loop continues at the convergence arm and never tests skip_edges (0 of 141 fixtures). Proposed simplification: drop the two lines. Not applied here -- deletion is a deliberate separate pass, and a skip hop that is not convergence-owned is not ruled out by contract. |
| 1364 | `if edge is None:` | `->L1365` |  |
| 1368 | `if family_id is None:` | `->L1369` |  |
| 1390 | `if system_id not in failures:` | `->L1394` |  |
| 1542 | `or route.route_system_disposition != RouteSystemDisposition.PLANNED.value` | `->L1544` | **needs-review** -- validate_member_geometry_emission' skip for a route that holds a frozen member geometry plan while its route system did not end up planned. build_member_geometry_execution drops every candidate of a system that fails, so a plan-holding route can only be non-planned when the system is escalated after the freeze -- which happens exactly when a convergence conflict escalates it to the compatibility disposition. No corpus system carries such a conflict, so the operand never decides. Reachable through the same fixture that would close any of this module's conflict arms. |
| 1545 | `if tuple(route.route_system_owned_segment_ranks) != plan.owned_segment_ranks:` | `->L1546` |  |
| 1551 | `if actual != (channel.start, channel.end):` | `->L1552` |  |
