...
| 1804 | `if max(peel_xs) - min(peel_xs) > (n - 1) * step + COORD_TOLERANCE:` | `->L1805` | **defensive** -- A complete destination bundle receives one contiguous n-slot channel band from gap materialization, so its peel-X extent cannot exceed (n-1)*step. The wider-band arm protects hand-built or corrupted route geometry and is exercised by test_invalid_opposing_candidate_is_rejected[noncontiguous-channel-band]. |
...
| 2290 | `elif tgt_sec:` | `->L2298` | **defensive** -- The else (col_key=round(sx)) needs an L-shaped (dx!=0,dy!=0) inter edge whose target resolves to no section. Only exit_port->junction edges lack a target section, and junction positioning makes those axis-aligned (same-X RIGHT exit or same-Y BOTTOM exit), never L-shaped, so the else is unreachable. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 2389 | `if dx > 0:` | `->L2392` | **defensive** -- Leftward (dx<=0) near-source fallback: the symmetric mirror of the rightward fallback. Reached only when section info is unavailable AND no adjacent column resolves; valid layouts take the rightward fallback, but the leftward mirror is kept for robustness. |
| 2400 | `if st is not None and st.is_port:` | `->L2398` | **defensive** -- Null/non-port guard: after _resolve_sections both endpoints of an inter-section edge are ports/junctions and graph.stations.get() resolves them, so the False (None or non-port) arm only guards a malformed pre-resolve graph. |
| 2449 | `if sec.bbox_w <= 0:` | `->L2450` | **defensive** -- Zero-width-section guard: the loop scans all graph.sections, but after compute_layout every placed section has bbox_w>0, so the skip arm only guards an empty/unplaced section that valid layouts don't produce. |
| 2497 | `if sec.bbox_w <= 0:` | `->L2498` | **defensive** -- Zero-width-section guard in the squeeze re-centring loop: like its #1 twin, after compute_layout every placed section has bbox_w>0, so the skip arm only guards an empty/unplaced section that valid layouts do not produce. |
| 2531 | `if not src.is_port:` | `->L2529` |  |
| 2547 | `for i in range(len(pts) - 1):` | `->L2548`, `->L2564` | **defensive** -- point_on_polyline is used only by the --animate renderer (render/animate.py); the matrix's render-path coverage sweep calls render_svg without animation, so none of its branches are reached. Exercised instead by test_animation.py / test_routing.py. |
| 2552 | `if seg_len2 == 0:` | `->L2553`, `->L2556` | **defensive** -- point_on_polyline degenerate-segment branch, reached only via the --animate renderer (render/animate.py); the render-path coverage sweep doesn't animate. Exercised by test_animation.py / test_routing.py, not the corpus. |
| 2553 | `if abs(point[0] - ax) <= tol and abs(point[1] - ay) <= tol:` | `->L2554`, `->L2555` | **defensive** -- point_on_polyline zero-length-segment hit test, reached only via the --animate renderer; the render-path coverage sweep doesn't animate. Exercised by test_animation.py / test_routing.py. |
| 2557 | `if t < -0.01 or t > 1.01:` | `->L2558`, `->L2559` | **defensive** -- point_on_polyline parameter-range branch, reached only via the --animate renderer; the render-path coverage sweep doesn't animate. Exercised by test_animation.py / test_routing.py. |
| 2562 | `if abs(point[0] - proj_x) <= tol and abs(point[1] - proj_y) <= tol:` | `->L2547`, `->L2563` | **defensive** -- point_on_polyline projection hit test, reached only via the --animate renderer; the render-path coverage sweep doesn't animate. Exercised by test_animation.py / test_routing.py. |
| 2657 | `if all_in_range:` | `->L2661` | **defensive** -- cross_row bypass guard: the False arm (no sections in the column range -> return clearance) needs an empty [lo,hi] column span, which a valid multi-section pipeline routing cross_row never has; a degenerate-range guard. |
| 2684 | `if endpoints:` | `->L2687` | **defensive** -- Non-cross_row bypass guard: the endpoints list always includes the source and target sections (both in src_row), so it is never empty; the default-clearance arm only guards a degenerate empty endpoint set. |
| 2697 | `if src_row is not None:` | `->L2709` | **defensive** -- Header-clamp guard: the False arm (src_row is None) needs an unresolvable source section, but inter-section bypass sources always resolve to a grid row after layout; a guard against an unresolved source that doesn't occur. |
| 2704 | `if safe_cap >= row_bottom:` | `->L2707` | **defensive** -- The false arm is a defensive midpoint fallback for an inter-row gap too narrow to clear both an upper-row bypass and the lower section header. Placement reserves the routing band before bypass geometry is computed, so settled corpus layouts provide enough clearance for the safe-cap arm. |
| 2803 | `if section.bbox_w <= 0:` | `->L2804` | **defensive** -- Degenerate-bbox guard in _h_segment_penetrates_section (the single-section predicate shared by _h_segment_crosses_other_section and _packed_cell_mate_obstructs): sections always have positive width after layout. |
| 2886 | `if station is None:` | `->L2887` | **defensive** -- None-station guard: every production caller passes a concrete Station (grep-verified); the None arm only guards the typed Station\|None parameter. |
| 2892 | `for port in junction_source_ports(graph, station.id):` | `->L2911` | **defensive** -- Upstream loop-exit in resolve_section: every fan-out junction is emitted with an exit_port->junction edge whose source port carries a section id, so junction_source_ports always yields a sectioned port and the loop returns; the fall-through to the type-contract return None is the never-taken no-upstream fallback. |
| 2894 | `if sec:` | `->L2892` | **defensive** -- Upstream section-lookup guard: graph.sections.get(section_id) returning None requires a section_id absent from graph.sections, a corrupted state no valid parse produces; null-guard. |
| 2900 | `for e in graph.edges:` | `->L2911` | **defensive** -- No-preference graph.edges scan loop-exit: a fan-out junction always has a sectioned incident port (its exit/entry ports carry a section_id), so the loop always returns and never exhausts graph.edges; the fall-through to the type-contract return None is the never-taken no-neighbour fallback. |
| 2901 | `if e.source == station.id:` | `->L2902` | **defensive** -- Junction-as-source arm of the no-preference scan: fan-out junctions are emitted exit_port->junction before junction->entry_port, so graph.edges encounters the junction as a target (elif arm) first and returns; the source arm is a cheap structural guard whose deadness rests on emit order, not a hard type invariant. |
| 2907 | `if other.section_id:` | `->L2900` | **defensive** -- The prefer_upstream=False scan reads a junction's incident neighbour on either side, always a sectioned station or port; the section-less (False) arm only guards a junction-to-junction edge valid construction never produces. |
| 2909 | `if sec:` | `->L2900` | **defensive** -- prefer_upstream=False section-lookup guard: graph.sections.get(section_id) returning None requires a missing section_id (corrupted state); null-guard. |
| 2919 | `if section is None:` | `->L2920` | **defensive** -- resolve_section_colrow accepts a station whose owning section may not resolve. Production routing passes resolved ports or junctions with an incident sectioned port, so the None return only protects a malformed or incomplete graph. |
| 2962 | `if reserved is None or abs(src_row - tgt_row) != 1:` | `->L2963` |  |
| 3018 | `if dy > 0:` | `->L3023` | **defensive** -- Multi-row upward-crossing arm: grid rows are assigned topologically and upward inter-section routes take other handlers (a same-row/around-section path), never reaching inter_row_channel_y's multi-row branch with dy<=0; verified by an upward multi-row fixture that bypasses this code. |
...
| 1454 | `if primary_delta * run_direction.sign < 2 * MIN_STRAIGHT_EDGE + diagonal:` | `->L1455` |  |
...
| 1646 | `if direction_axis(run_direction) is DemandAxis.Y:` | `->L1647` |  |
| 1672 | `if transition is None:` | `->L1673` |  |
...
| 3209 | `and corridor.bypass_band_y >= base_y - COORD_TOLERANCE` | `->L3219` | **needs-review** -- Safety fallback: the shared bypass band is the deepest sibling's bypass_bottom_y, so a branch's own base_y never exceeds it; the arm that would decline a shallower shared band (guarding a predicate mismatch) is unexercised in the corpus. |
| 3331 | `if trunk_v_up_pull_away:` | `->L3340` | **needs-review** -- Reachable only via a defective render (merge-junction tangle); see #721. |
| 3370 | `this_xmin - gap_left >= SECTION_ROUTE_CLEARANCE` | `->L3371`, `->L3389` |  |
| 3371 | `and gap_right - around_xmax >= SECTION_ROUTE_CLEARANCE` | `->L3373`, `->L3389` |  |
...
| 4147 | `while cur is not None and cur not in seen:` | `->L4163` |  |
| 4153 | `if port.side == PortSide.LEFT:` | `->L4155` |  |
| 4156 | `if cur in graph.junctions:` | `->L4162` |  |
| 4243 | `if (` | `->L4249` |  |
| 4428 | `if geometry is None:` | `->L4429` |  |
| 4527 | `if abs(tgt.x - src.x) <= ctx.curve_radius:` | `->L4528` |  |
| 4540 | `if _h_segment_crosses_other_section(graph, src.x, final_x, src.y, exclude):` | `->L4541` | **defensive** -- Traverse-first clearance test in _top_entry_side_fan_traverse_clears (#1349). The below-side fan branch only reaches this gate when its junction also feeds an aligned straight drop; the corpus fan sits a clear row above its target, so the source-Y traverse never crosses another section and the fall-back-to-staircase arm is not exercised. Defensive. |
| 4668 | `if abs(tgt.x - src.x) > COORD_TOLERANCE or src.id not in ctx.graph.junctions:` | `->L4669` |  |
| 4703 | `if feeder is not None:` | `->L4708` |  |
| 4878 | `if len(points) < 3:` | `->L4879` |  |
| 4930 | `if not section_ids:` | `->L4931` |  |
| 5010 | `if mid_y is None:` | `->L5027` |  |
| 5042 | `channel_y is None` | `->L5072` |  |
| 5044 | `and src_sec is not None` | `->L5072` | **defensive** -- Inter-row exit mid-Y clearance bump (the n>1 multi-line branch). When more than one line shares the exit the source section always resolves, so the `src_sec is None` short-circuit past the clearance bump is never taken. |
| 5045 | `and tgt_sec is not None` | `->L5072` | **defensive** -- Companion to src_sec in the same n>1 mid-Y clearance branch: the target section always resolves there, so the `tgt_sec is None` short-circuit past the bump is never taken. |
| 5074 | `if exit_side is not None:` | `->L5076` |  |
| 5076 | `elif abs(dx) > ctx.curve_radius:` | `->L5077`, `->L5079` |  |
| 5080 | `if src.id in ctx.graph.junctions:` | `->L5081`, `->L5093` | **needs-review** -- Reachable only via a defective render (LR->TB TOP-entry routing); see #720. |
| 5081 | `for je in ctx.graph.edges_to(src.id):` | `->L5082`, `->L5093` |  |
| 5083 | `if js.is_port:` | `->L5081`, `->L5084` |  |
| 5084 | `if abs(js.x - src.x) <= COORD_TOLERANCE:` | `->L5085`, `->L5087` |  |
| 5099 | `if exit_side is not None and not straight_drop:` | `->L5109` |  |
| 5147 | `if not straight_drop:` | `->L5151` | **needs-review** -- A top-entry fan branch fed straight from directly above the junction (straight_drop) keeps its lead-in column at the source X rather than the shared fan pivot; the corpus's fan branches all carry horizontal travel, so the straight-drop arm is unexercised. |
| 5149 | `if lead is Direction.R:` | `->L5151` | **needs-review** -- A top-entry fan branch leading LEFT out of the junction skips the _v1_corner_x right-edge clearance bump; the corpus's fan branches all lead right, so the left-lead arm is unexercised. |
| 5176 | `if straight_drop:` | `->L5177` |  |
| 5253 | `if ctx.exit_turns is None:` | `->L5254` | **needs-review** -- _perp_entry_turn_is_planned answers False for the pre-plan context the exit-turn planner probes emitters with; ctx.exit_turns is bound only for emission. That context does reach these emitters -- the same guard in _seat_bypass_descent takes its None arm on packed_cell_right_exit_left_entry_wrap -- but no corpus probe reaches the TOP/BOTTOM-entry L-shape records this predicate feeds. |
| 5338 | `if src_sec is None or tgt_sec is None or src_sec.grid_row >= tgt_sec.grid_row:` | `->L5341` |  |
| 5508 | `if src_section and src_section.bbox_w > 0:` | `->L5509` |  |
| 5529 | `if tgt_sec is None:` | `->L5530` |  |
| 5600 | `if src_col is None:` | `->L5601` |  |
| 5835 | `if src_sec is not None:` | `->L5866` | **defensive** -- Defensive guard in the fanned entry-wrap source-clearance lift: a fan-out junction always resolves to its source section, so the None arm is unreachable in practice. |
| 5872 | `if tgt_col is not None:` | `->L5877` | **defensive** -- tgt_col from _resolve_section_col on a valid LEFT entry port is always non-None (sections get grid_col>=0 after auto-layout). |
| 5874 | `if shared_vx is not None:` | `->L5877` | **defensive** -- _fan_left_entry_descent_x returns None only when col_left<=0; col_left_edge for the target's own column is always >0. |
| 6000 | `if entry_side is PortSide.LEFT:` | `->L6003` |  |
| 6068 | `if entry_port is None:` | `->L6069` | **defensive** -- _route_around_section_below is always called with a non-None entry_port (both dispatch sites pass a checked station). |
| 6079 | `if trunk_src is None or trunk_src == edge.source:` | `->L6081` | **needs-review** -- Reachable only via a defective render (merge-junction tangle); see #721. |
| 6132 | `if col_left <= 0.0:` | `->L6133` | **defensive** -- col_left_edge's 0.0 default only fires for a column with no sections; the target's own column always holds its section. |
| 6169 | `if entry_port is None:` | `->L6170` | **defensive** -- Every caller of _corridor_is_viable passes a non-None entry_port. |
| 6176 | `if src_row is None or ep_row is None or src_col is None or ep_col is None:` | `->L6177` | **defensive** -- _resolve_section_colrow returns (None,None) only for a station with no section; valid entry ports always have a section with non-negative grid coords. |
...
| 7071 | `if gap_bottom <= gap_top:` | `->L7072` |  |
| 7073 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L7074` |  |
| 7084 | `if f.v_segment_crosses_other_section(vx, gy, tgt.y):` | `->L7085` |  |
| 7179 | `if source is None or source.bbox_w <= 0:` | `->L7180` |  |
| 7184 | `if source.id not in members:` | `->L7185` |  |
| 7190 | `if other.bbox_x <= source_right + COORD_TOLERANCE:` | `->L7191` |  |
| 7192 | `if cellmate_left is None or other.bbox_x < cellmate_left:` | `->L7186` |  |
| 7194 | `if cellmate_left is None:` | `->L7195` |  |
| 7232 | `if band0_bottom <= band0_top or not _inter_row_band_fits(band0_top, band0_bottom):` | `->L7233` |  |
| 7234 | `if band1_bottom <= band1_top or not _inter_row_band_fits(band1_top, band1_bottom):` | `->L7235` |  |
| 7236 | `if band1_top <= band0_bottom + COORD_TOLERANCE:` | `->L7237` |  |
| 7252 | `if f.v_segment_crosses_other_section(corner_x, band0_y, band1_y, exclude):` | `->L7253` |  |
| 7309 | `if geom is None:` | `->L7310` |  |
| 7312 | `if f.v_segment_crosses_other_section(geom.lead_x, src.y, geom.band0_y):` | `->L7313` |  |
| 7314 | `if f.h_segment_crosses_other_section(geom.lead_x, geom.corner_x, geom.band0_y):` | `->L7315` |  |
| 7316 | `if f.h_segment_crosses_other_section(geom.corner_x, geom.vx, geom.band1_y):` | `->L7317` |  |
