...
**2204/3134** gates fully exercised (both/all arms hit by some fixture); **930** gates have at least one un-exercised arm.
...
| 3994 | `held[0] - COORD_TOLERANCE < coord < held[1] + COORD_TOLERANCE` | `->L3995` |  |
| 3995 | `and span[0] < crossed[1]` | `->L3987`, `->L3996` | **defensive** -- Crossed-axis overlap test in _run_enters_section, reached only for a target already bracketed on the held axis. No corpus separation target is, so this operand's short-circuit arm never fires. |
| 3996 | `and crossed[0] < span[1]` | `->L3987`, `->L3998` | **defensive** -- Second half of the crossed-axis overlap test in _run_enters_section, reached only once the first half holds. No corpus separation target reaches it, so its short-circuit arm never fires. |
