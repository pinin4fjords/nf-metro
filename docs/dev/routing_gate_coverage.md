...
| 526 | `if lane_is_clear_in_corridor(` | `->L530` | **needs-review** -- The straight connector's lane hand-off is placed at the source end when its source lane is already occupied in the target's approach corridor: the source-corridor test returns False and falls through to the target-corridor test at line 534. Only variantbenchmarking_auto reached this False arm, via its ensembl_truth->benchmarking 'truth' connector whose source lane the 'test' line shared in the entry corridor. #1844 seats that partial row-mate one handover lane deeper so the connector runs flat, so its run is no longer a lane transition and _place_hand_off_at_source is not reached. The arm stays reachable for a same-Y port-to-port straight connector whose source lane a co-arriving line occupies without a partial-trunk descent flattening it; no swept fixture currently presents one. |
