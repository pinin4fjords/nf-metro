...
**1944/2815** gates fully exercised (both/all arms hit by some fixture); **871** gates have at least one un-exercised arm.
...
| 3425 | `assignment is None` | `->L3430` |  |
| 3426 | `or membership.axis is None` | `->L3430` |  |
| 3427 | `or assignment.run_direction is None` | `->L3430` |  |
| 3428 | `or assignment.turn_direction is None` | `->L3430` |  |
| 3473 | `if ctx.exit_turns is None:` | `->L3474` |  |
| 3484 | `if family_id is not assignment.planned_family_id:` | `->L3485` |  |
| 3497 | `if (` | `->L3506` |  |
| 3515 | `run is None` | `->L3520` |  |
| 3516 | `or turn is None` | `->L3520` |  |
| 3517 | `or assignment.launch_coordinate is None` | `->L3520` |  |
| 3518 | `or assignment.minimum_runway is None` | `->L3520` |  |
| 3524 | `if segment_rank is None:` | `->L3525` |  |
| 3576 | `if source_turn_changed and settled is None:` | `->L3577` |  |
| 3606 | `if rank is None:` | `->L3607` |  |
| 3673 | `if current.keys() != snapshot.geometry.keys():` | `->L3674` |  |
| 3688 | `if current[key] != state:` | `->L3689` |  |
| 3721 | `station_id not in graph.stations` | `->L3724` |  |
| 3722 | `or lane.line_id not in graph.station_lines(station_id)` | `->L3724` |  |
| 3730 | `if (station_id, lane.line_id) not in station_offsets or (` | `->L3737` |  |
| 3745 | `if len(transition_routes) != 1:` | `->L3746` |  |
| 3783 | `route.exit_lane_transition_plan_id != str(exit_turn_plan.id)` | `->L3788` |  |
| 3784 | `or route.offset_regime is not OffsetRegime.BAKED` | `->L3788` |  |
| 3785 | `or route.points != expected_route.points` | `->L3788` |  |
| 3786 | `or route.curve_radii != expected_route.curve_radii` | `->L3788` |  |
| 3804 | `if not _fixed_axis_matches_plan(` | `->L3809` |  |
| 3815 | `if any(` | `->L3824` |  |
| 3832 | `if len(member_routes) != 1:` | `->L3833` |  |
| 3840 | `if route.exit_turn_family_id != assignment.planned_family_id.value:` | `->L3841` |  |
| 3847 | `if route.route_system_disposition == "compatibility":` | `->L3848` |  |
| 3850 | `if route.exit_turn_axis_id is not None:` | `->L3851` |  |
| 3859 | `if (` | `->L3873` |  |
| 3881 | `route.exit_turn_axis_id != str(assignment.axis_id)` | `->L3884` |  |
| 3882 | `or route.exit_turn_segment_rank is None` | `->L3884` |  |
| 3895 | `if (` | `->L3917` | **defensive** -- The emission validation that raises when a rendered turn's axis, lead-in runway or direction differs from the assignment the route was bound to. The emitters derive all four from that same assignment, so the raise names an emitter ignoring its plan rather than a topology. |
...
| 761 | `if ctx.exit_turns is None:` | `->L762` |  |
| 784 | `if planned_offsets is None:` | `->L785` |  |
| 804 | `dx is None` | `->L808` |  |
| 806 | `or not 0 <= radius_index < len(route.curve_radii)` | `->L808` |  |
| 813 | `if (` | `->L817` |  |
| 848 | `if gap_right <= gap_left:` | `->L849` |  |
| 992 | `go is not None` | `->L998` |  |
| 993 | `and gt is not None` | `->L998` |  |
| 1030 | `if len(pts) < 2:` | `->L1031` |  |
| 1046 | `if abs(y2 - y1) > COORD_TOLERANCE or abs(x2 - x1) <= COORD_TOLERANCE:` | `->L1047` |  |
| 1123 | `if slot is None or trunk is None or trunk[0] != k:` | `->L1124` | **needs-review** -- _reconcile_moved_trunk_slot's precondition guard: the re-banded leg is not the route's first interior horizontal run, or the route declared no trunk slot. The docstring states this as a real filter ('only the trunk the slot describes can retarget it'), and _bundle_divergent_distinct_traverses hands it every distinct-line traverse leg, so a non-trunk leg or a slotless route reaches it; all 3 corpus fixtures happen to pass. Reachable; wants a fixture. |
| 1134 | `if any(abs(channel.x - ref_x) > COORD_TOLERANCE for channel in planned[1:]):` | `->L1135` |  |
| 1135 | `if ctx.validate_final_route_frames:` | `->L1136`, `->L1137` |  |
| 1181 | `if channel is None:` | `->L1182` |  |
| 1254 | `if planned and max(planned) - min(planned) > COORD_TOLERANCE:` | `->L1255` |  |
| 1260 | `if ref is not None:` | `->L1261` |  |
| 1327 | `for group in _merge_feeder_groups(routes, ctx):` | `->L1330` |  |
| 1336 | `if compatibility_channels:` | `->L1327`, `->L1337` |  |
| 1374 | `if not downs or not ups:` | `->L1376` |  |
| 1377 | `for descent in downs:` | `->L-1343`, `->L1378` |  |
| 1379 | `if col in moved:` | `->L1380`, `->L1381` |  |
| 1382 | `for up in ups:` | `->L1383`, `->L1392` |  |
| 1383 | `if up.route.line_id != descent.route.line_id:` | `->L1384`, `->L1385` | **defensive** -- _clear_merge_trunk_opposite_arm skips an up-arm carrying a different line from the down-trunk, since only a same-line opposite arm folds the line back over itself. The corpus's one fork sends a single line both ways, so the different-line skip arm stays un-exercised. Defensive against a fork whose opposite arms carry distinct lines. |
| 1385 | `if abs(up.x - descent.x) > radius:` | `->L1386`, `->L1387` |  |
| 1388 | `if overlap <= COORD_TOLERANCE:` | `->L1389`, `->L1390` | **defensive** -- _clear_merge_trunk_opposite_arm skips an opposite arm within a curve radius of the down-trunk but not overlapping it in Y, since without a shared Y span there is no fold-back to clear. The corpus's qualifying fork overlaps in Y, so the no-overlap skip arm stays un-exercised. Defensive against a near-column opposite arm that clears the trunk vertically. |
| 1392 | `if target_x is None:` | `->L1393`, `->L1394` |  |
| 1395 | `for ch in downs:` | `->L1377`, `->L1396` |  |
| 1396 | `if abs(ch.x - descent.x) <= COORD_TOLERANCE:` | `->L1395`, `->L1397` | **defensive** -- _clear_merge_trunk_opposite_arm moves every down-leg sharing the folding descent's column and skips those on other columns. The corpus's one folding descent carries all its down-legs on a single shared column, so the other-column skip arm stays un-exercised. Defensive against a fork with down-legs spread over several columns. |
| 1423 | `if k == desc.idx + 1:` | `->L1422` | **defensive** -- In _fanout_traverse_spans: the corpus fan-out routes carry the traverse as their only interior horizontal trunk, so the loop matches on the first trunk and the non-match arm is never taken. Defensive. |
| 1467 | `if lower > upper + COORD_TOLERANCE_FINE:` | `->L1468` |  |
| 1479 | `if any(` | `->L1489` |  |
| 1515 | `abs(p0[1] - p1[1]) > COORD_TOLERANCE` | `->L1519` |  |
| 1516 | `or abs(p1[0] - p2[0]) > COORD_TOLERANCE` | `->L1519` |  |
| 1517 | `or abs(p2[1] - p3[1]) > COORD_TOLERANCE` | `->L1519` |  |
| 1536 | `if source_col is None:` | `->L1537` |  |
| 1552 | `if route_system_owns_segment_boundary(route, opening.idx):` | `->L1553` |  |
| 1569 | `if not sibling_xs:` | `->L1570` |  |
| 1572 | `if new_x <= max(sibling_xs) + ctx.curve_radius + COORD_TOLERANCE:` | `->L1573` |  |
| 1575 | `if _section_intrudes(` | `->L1582` |  |
| 1602 | `if any(` | `->L1606` |  |
| 1620 | `if len(turns) != 1:` | `->L1621` | **defensive** -- In _bundle_divergent_distinct_traverses: every fan in the corpus turns its traverses the same way, so the mixed-direction skip is never taken. Defensive. |
...
| 1684 | `elif ctx.merge.entry_port_for.get(rp.edge.source) == rp.edge.target:` | `->L1685` |  |
| 1695 | `for merge_id, hop in hop_by_merge.items():` | `->L1696` |  |
| 1697 | `if not feeders or any(ends_at(route, hop.points[0]) for route in feeders):` | `->L1698`, `->L1699` |  |
| 1702 | `if carrier is None:` | `->L1703`, `->L1704` | **defensive** -- A removable merge-entry hop has feeders, none ending at the hop start, and one ending at the hop destination after feeder normalization. A missing carrier therefore signals broken settled geometry; the guard keeps the hop instead of discarding a visible route. |
| 1711 | `if not covered:` | `->L1713` |  |
| 1767 | `for _ in range(max(1, len(buckets))):` | `->L-1717` | **defensive** -- Bounded fixed-point repair for coincident corner radii. Production routes settle without a second correction pass; the multi-bucket propagation and loop-exhaustion paths are exercised directly by test_unify_refreshes_a_shared_route_after_changing_one_corner. |
| 1785 | `if actual - common <= COORD_TOLERANCE_FINE:` | `->L1787` | **defensive** -- Production coincident turns have enough shared runway after planning and already resolve to the same radius. The correction arm is a defensive repair for directly supplied or future route geometry and is exercised by the coincident-corner unit tests. |
| 1790 | `for _step in range(64):` | `->L1791`, `->L1798` | **defensive** -- Fixed-iteration bisection used only when a defensive coincident-radius repair must lower one route. The correction and completed bisection are exercised by test_unify_uses_widest_radius_all_shared_legs_can_resolve. |
| 1794 | `if resolved < common:` | `->L1795`, `->L1797` | **defensive** -- Both bisection directions belong to the defensive coincident-radius repair rather than ordinary topology dispatch. The short-runway unit case exercises the search and verifies the final common effective radius. |
| 1801 | `if not changed:` | `->L1767` | **defensive** -- The production corpus reaches the immediate fixed point because coincident members already share an effective radius. The continue-for-propagation arm is a defensive repair path covered by test_unify_refreshes_a_shared_route_after_changing_one_corner. |
| 1836 | `if _planner_owns_channel(ch):` | `->L1837` |  |
| 1900 | `if opposing_bundle is not None and {` | `->L1903` | **defensive** -- iter_opposing_entry_confluences requires the complete direct-port line set, while by_port can only add a member through convergence-junction aliasing. Resolved convergence junctions replace the direct approach rather than coexist with a complete direct-port bundle, so unequal ownership sets require an inconsistent route/context pair. test_opposing_group_with_an_extra_mapped_approach_uses_generic_ownership exercises the defensive fallback. |
| 1910 | `if any(` | `->L1919` |  |
| 1991 | `if any(` | `->L1996` |  |
| 2032 | `if has_planned_channel and any(` | `->L2037` |  |
| 2088 | `if any(` | `->L2092` |  |
| 2093 | `if not crossing:` | `->L2100` |  |
| 2103 | `if abs(delta) <= COORD_TOLERANCE:` | `->L2104`, `->L2108` | **defensive** -- _snap_merge_feeder_group skips a merge feeder whose opening descent already sits on the trunk's shared column (no shift needed). The corpus's merge feeders all reach the trunk from a distinct column, so the already-coincident skip arm stays un-exercised. Defensive and idempotent across the routing replays a render performs. |
| 2108 | `for r, k, _y in crossing:` | `->L2065`, `->L2109` |  |
| 2230 | `if rp.curve_radii is None:` | `->L2231` | **defensive** -- curve_radii is always a concrete list for inter-section routes (every inter-section handler sets it); the None arm in _reseat_concentric_flanking is a belt-and-suspenders guard. |
| 2429 | `if route.curve_radii is None:` | `->L2430` | **defensive** -- Fan-opening channels are selected from a real horizontal-to-vertical turn, and production turn builders attach radius metadata to that shape. The None arm protects a synthetic or incomplete RoutedPath. |
| 2496 | `if port is not None and port.is_entry:` | `->L2498` |  |
| 2520 | `if (` | `->L2527` |  |
| 2539 | `if any(` | `->L2543` |  |
| 2651 | `if not trunks:` | `->L2652`, `->L2653` | **defensive** -- Shape guard in _merge_convergence_run: a trunk routed as a U-shaped bypass always has an interior horizontal leg for iter_horizontal_trunks to find, so the corpus never takes the None arm. Not provably unreachable across every branch of _route_bypass, so it stays a guard rather than an assert (#1597). |
| 2678 | `if not verticals:` | `->L2679` | **defensive** -- Shape guard in _land_feeder_on_run: a merge branch feeder is built as a descent into a tail, so it always has a vertical leg. Reachable in principle only if a feeder's source Y coincides with the trunk channel level, collapsing the descent below COORD_TOLERANCE (#1597). |
| 2683 | `if k + 2 != len(pts) - 1 or y_hi - y_lo < radius:` | `->L2684` | **defensive** -- Shape guard in _land_feeder_on_run. _route_merge_branch emits exactly four waypoints, while an alternate attached-source construction or a malformed hand-built route can add points; the second half rejects a descent too short to re-form its corner. Neither shape occurs in the corpus (#1597). |
| 2694 | `if rp.curve_radii is not None:` | `->L2696` | **defensive** -- Truncating the tail in _land_feeder_on_run keeps curve_radii aligned with the shortened waypoint list when metadata is present. Hand-built and compatibility routes may omit that metadata, so the field cannot be treated as always present (#1597). |
| 2722 | `if len(rp.points) != 2:` | `->L2723`, `->L2724` |  |
| 2725 | `if abs(lane - _get_offset(ctx, rp.edge.target, rp.line_id)) <= COORD_TOLERANCE:` | `->L2726`, `->L2727` |  |
| 2732 | `if not (riser_spans_lane and riser_ahead):` | `->L2733`, `->L2734` | **defensive** -- Reachability guard in _land_lane_changing_feeder_on_trunk_riser: the trunk's riser has to cross the feeder's lane and stand in the feeder's direction of travel before the feeder can terminate on it. Only one corpus fixture reaches the guard at all (exit_run_three_drop_columns, the sole adjacent merge feeder whose direct run changes lane), and there the riser both spans the lane and stands ahead, so the reject arm needs a trunk that turns before the feeder junction or lands short of its lane. |
| 2756 | `if trunk_rp.convergence_plan_id is not None:` | `->L2758` |  |
| 2759 | `if run is None:` | `->L2760`, `->L2761` | **defensive** -- Consumer of the _merge_convergence_run None arm above; unreachable for exactly the same reason and kept for the same reason (#1597). |
| 2761 | `for rp in others:` | `->L2755`, `->L2762` |  |
| 2762 | `if (rp.edge.source, rp.edge.target, rp.line_id) in merge.branch_edges:` | `->L2763`, `->L2765` |  |
| 2981 | `elif far_y < t.y - COORD_TOLERANCE:` | `->L2975` | **defensive** -- Degenerate-leg guard in _trunk_slot_features: a trunk's flanking legs are genuine vertical legs (far_y differs from the trunk y), so neither the below nor the above classification is skipped; the fall-through arm needs a zero-height leg the layout never produces. |
| 3011 | `if feats is None:` | `->L3012` | **defensive** -- Unused default-arg branch: all three callers of _band_order_crossings pass feats explicitly, so the feats-is-None compute-it arm is never taken. |
| 3181 | `if len(trunks) < 2:` | `->L3182`, `->L3183` | **defensive** -- Early-return in _suboptimal_trunk_bands, called only from the _guard_* validator (phases/guards.py) and a unit test, never on the production render path. The coverage sweep renders with validate=False, so neither arm registers. Defensive (exercised via the validate=True guard / test surface; cf. #728 validate-only precedent). |
| 3185 | `for grp in groups:` | `->L3186`, `->L3204` | **defensive** -- _suboptimal_trunk_bands diagnostic loop: validate-only, not reached by the render-path coverage sweep (see the len(trunks)<2 note). |
| 3186 | `if len({id(t.route) for t in grp}) < 2:` | `->L3187`, `->L3188` | **defensive** -- Single-route-group skip in _suboptimal_trunk_bands, validate-only: called from the _guard_* validator and a unit test, not the production render path the validate=False sweep exercises. Defensive. |
| 3188 | `if not any(not t.route.normalize_exempt for t in grp):` | `->L3189`, `->L3190` | **defensive** -- _suboptimal_trunk_bands diagnostic guard: validate-only, not on the render path. |
| 3190 | `for sign in (1, -1):` | `->L3185`, `->L3191` | **defensive** -- _suboptimal_trunk_bands direction loop: validate-only, not on the render path. |
| 3193 | `if len(slots) < 2 or len(slots) > _MAX_BAND_PERMUTE:` | `->L3194`, `->L3195` | **defensive** -- _suboptimal_trunk_bands slot-count guard: validate-only, not on the render path. |
| 3202 | `if best < cur:` | `->L3190`, `->L3203` | **defensive** -- _suboptimal_trunk_bands crossing-comparison: validate-only, not on the render path. |
| 3369 | `if band is not None:` | `->L3375` | **defensive** -- Same-line dogleg: the band-found arm is exercised by dogleg_exempt_sameline; the band-is-None arm is defensive (both colliding trunks route through _center_inter_row_channel, so the trunk Y always resolves to a gap band). |
| 3390 | `if down_ok and up_ok and (cross_down is None) != (cross_up is None):` | `->L3392` | **defensive** -- Same-line dogleg side selection. The crossing-decided true arm (->1297, pick the side whose riser does not pierce the exempt run) is exercised by dogleg_exempt_sameline. The false fall-through (->1298) needs either an out-of-band side or a crossing-equal tie, neither reachable by a clean topology: the reachable dogleg regime rigidly fixes the inter-row band so down_room/up_room stay >= min_sep=6 (observed 14/26 across the corpus and every constructed variant; perturbing the grid loses the regime), and the only realizable tie is both-cross (a crossing-free tie needs degenerate zero-length exempt risers) which co-occurs with section-crossing / dogleg-crosses-exempt guard failures and is never a shippable render. |
| 3392 | `elif down_ok and (not up_ok or prefer_down):` | `->L3393`, `->L3394` | **defensive** -- Same-line dogleg tie / out-of-band fallback, reached only after the L1296 if is false, which a clean topology never produces (see that gate): the true arm (->1299, lean down on a tie) needs a crossing-free tie that requires degenerate risers, and the false arm (->1300) needs down_ok False, which the regime's fixed inter-row band (down_room >= min_sep) never yields. |
| 3394 | `elif up_ok:` | `->L3395`, `->L3397` | **defensive** -- Same-line dogleg move-up / give-up fallback, reached only when both L1296 and L1298 are false. The true arm (->1301, move up because the lower gap is too tight) needs down_ok False and the else arm (->1303, leave fused) needs both rooms below min_sep; the reachable regime's inter-row band is auto-widened so both rooms stay >= min_sep=6 (observed 14/26), so neither is reached by a valid topology. |
...
| 3437 | `if below_ok and above_ok and (cross_below is None) != (cross_above is None):` | `->L3439` | **defensive** -- Distinct-line dogleg side selection. The crossing-decided true arm (->1342) is exercised by dogleg_exempt_distinct. The false fall-through (->1343) needs a crossing-equal tie; a crossing-free tie requires degenerate zero-length exempt risers and the only realizable tie is both-cross, which trips _guard_no_dogleg_crosses_exempt_trunk plus the section-crossing guards and is never a shippable render. |
| 3439 | `elif below_ok and (not above_ok or prefer_below):` | `->L3440`, `->L3441` | **defensive** -- Distinct-line dogleg tie fallback, reached only after L1341 is false (a tie), which a clean topology never produces (see that gate). The true arm (->1344, lean below) would fire on such a tie; the false arm (->1345) is unreachable because below_ok always holds and prefer_below is always true (t.y == hit.y in this regime). |
| 3441 | `elif above_ok:` | `->L3442`, `->L3444` | **defensive** -- Distinct-line dogleg move-up/skip branch: unreachable because the below branch is always taken (t.y == hit.y, below_ok true); covers both the move-up and the gap-too-tight continue. |
| 3459 | `held[0] - COORD_TOLERANCE < coord < held[1] + COORD_TOLERANCE` | `->L3460` | **defensive** -- Held-axis bracket in _run_enters_section, the section-intrusion veto's first test. No corpus separation target lands within a section's extent on the held axis, so the inside arm never fires; the same defensive reasoning as the veto it serves. |
| 3460 | `and span[0] < crossed[1]` | `->L3452`, `->L3461` | **defensive** -- Crossed-axis overlap test in _run_enters_section, reached only for a target already bracketed on the held axis. No corpus separation target is, so this operand's short-circuit arm never fires. |
| 3461 | `and crossed[0] < span[1]` | `->L3452`, `->L3463` | **defensive** -- Second half of the crossed-axis overlap test in _run_enters_section, reached only once the first half holds. No corpus separation target reaches it, so its short-circuit arm never fires. |
| 3530 | `if i in relocated:` | `->L3531` | **defensive** -- Re-entry skip in the co-travelling separation worklist. A track is re-queued only while it is still un-relocated, so the second copy of a track queued twice before its first pop is the only way to reach the skip; across the corpus every re-queued track is popped before it moves, so the taken arm never fires. Defensive against a crowded stack whose neighbours re-queue one track twice, where taking it twice would break the one-move-per-track bound. |
| 3541 | `if any(` | `->L3545` | **needs-review** -- Never re-band a fan traverse across a section it does not belong to: the re-band would draw the traverse through an unrelated box. The corpus reached the crossing arm only through merge_trunk_over_low_section's pre-fix geometry, where a group carrying a member-geometry-owned channel was skipped and its traverses kept coordinates that crossed a foreign section. With the group gated on the coordinates a plan resolves outright, no fixture crosses one, so the guard keeps a real hazard with no corpus witness. |
| 3587 | `if sg[0].sign_x != t.sign_x:` | `->L3588` | **defensive** -- Both callers of _coincident_trunk_slots pre-filter input to a single traversal direction (band = [t for t in grp if t.sign_x == sign]), so within the function sg[0].sign_x != t.sign_x is always False; the opposing-direction continue is unreachable. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3618 | `if rp.curve_radii is None:` | `->L3619` | **defensive** -- curve_radii is always set for inter-section routes; the None arm in _restack_htrunk is a defensive guard. |
| 3630 | `if 0 <= k - 1 < len(rp.curve_radii):` | `->L3632` | **defensive** -- Bounds guard in _restack_htrunk: trunk index k >= 1 so k-1 >= 0 is always in range; the skip arm is defensive. |
| 3632 | `if k < len(rp.curve_radii) and k + 2 < len(pts):` | `->L-3597` | **defensive** -- Bounds guard in _restack_htrunk: for a real interior trunk k and k+2 are in range; the skip arm is defensive. |
| 3691 | `if down is None or len(up.points) < 2:` | `->L3692` | **needs-review** -- Un-exercised on origin/main too; 897747b0 covered it on 13 fixtures and HEAD is back to main's [0,138] split, so this is a return to main's behaviour rather than a new gap. Reachable by construction (main's baseline lists it as an open gap already). No fixture authored -- the render corpus is frozen for a measurement against main. |
| 3737 | `if len(targets) != 1:` | `->L3738` |  |
| 3747 | `if col is None:` | `->L3748` | **defensive** -- Defensive guard in _convergence_line_order: a peel-off riser channel always resolves to a source-section column, so the unresolved-column early return is never taken on the corpus. |
| 3788 | `if not 0 <= far < len(pts):` | `->L3789` |  |
| 3791 | `if abs(dx) > COORD_TOLERANCE:` | `->L3785` |  |
| 3793 | `if not votes:` | `->L3794` |  |
| 3897 | `if ca != cb:` | `->L3899` |  |
| 3899 | `if rep_x[a] != rep_x[b]:` | `->L3900`, `->L3901` | **defensive** -- The routing system assigns distinct x offsets to every line sharing a column gap, so two distinct lines in one gap-bundle corridor never share a representative x; the equal-rep_x tiebreak fall-through in _distinct_line_order is unreachable (exhaustive corpus scan confirms). Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3929 | `if rp.curve_radii is None:` | `->L3930` | **defensive** -- curve_radii is always set for inter-section routes; the None arm in _restack_channel is a defensive guard. |
| 3932 | `if not 0 <= radius_idx < len(rp.curve_radii):` | `->L3933` |  |
| 3956 | `if abs(ly - pts[1][1]) < COORD_TOLERANCE:` | `->L-3906` | **defensive** -- A re-stacked gap channel at points-index 1 always has a horizontal lead-in (L-shape and bypass routes open with a horizontal segment); routes opening non-horizontally are normalize_exempt and skipped by the materialization. The non-horizontal-lead return in _restack_channel is unreachable. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3986 | `if anchor_section_id is not None and anchor_side is not None:` | `->L3988` | **defensive** -- _gap_channel_base falls back to the column gap when passed a null anchor. All six call sites (in _route_bypass) pass the bypass source/target section id, which is None only when that endpoint has no resolvable section (src_sec_id/tgt_sec_id in _route_bypass); the corpus has no bypass with a sectionless endpoint, so the null-anchor arm is never taken. Defensive guard for a sectionless bypass endpoint. |
| 4041 | `elif right_ok:` | `->L4044` |  |
| 4090 | `if s.bbox_w <= 0:` | `->L4091` | **defensive** -- Degenerate-bbox guard in _v_segment_crosses_other_section: mirror of the _h_segment_penetrates_section guard; sections always have positive width. |
| 4470 | `if not blockers:` | `->L4471` |  |
...
| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 143 | `if scaffold is None:` | `->L144` |  |
| 249 | `if station_offsets is not None:` | `->L252` |  |
