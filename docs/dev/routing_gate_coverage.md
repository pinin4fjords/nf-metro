...
**1946/2831** gates fully exercised (both/all arms hit by some fixture); **885** gates have at least one un-exercised arm.
...
184/291 gates fully exercised.

Gates with an un-exercised arm:

| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 490 | `if not replacements:` | `->L491`, `->L492` |  |
| 721 | `if key not in ctx.station_offsets and port_key not in ctx.station_offsets:` | `->L722` |  |
| 734 | `if len({coordinate for _line, _offset, coordinate in values}) != len(values):` | `->L735` |  |
| 807 | `if source_run_direction not in {Direction.R, Direction.L}:` | `->L808` |  |
| 897 | `if source_run_direction not in run_axis:` | `->L898` |  |
| 902 | `if abs(delta) <= COORD_TOLERANCE:` | `->L903` |  |
| 906 | `if horizontal:` | `->L909` |  |
| 910 | `if actual_run is not source_run_direction:` | `->L911` |  |
| 939 | `if route is None:` | `->L940` |  |
| 942 | `if len(points) < 3:` | `->L943` |  |
| 947 | `if run_direction is None or turn_direction is None:` | `->L948` |  |
| 974 | `if off_column_decline is not None and source_run_direction not in {` | `->L978` |  |
| 979 | `if family_id is RouteFamilyId.MERGE_ENTRY_STRAIGHT:` | `->L980` |  |
| 1072 | `if entry_geometry.seam.turn_direction is None:` | `->L1073` |  |
| 1156 | `if abs(turn_delta) <= COORD_TOLERANCE:` | `->L1157` |  |
| 1184 | `if abs(turn_delta) <= COORD_TOLERANCE:` | `->L1185` | **defensive** -- _plain_l_shape_turn_requirement's flat-hop guard, which both its callers exclude: _standard_l_shape_turn_requirement returns its own missing-source-turn for a flat delta before delegating, and the cell-mate gap drop only classifies when the target section sits one grid row below the source (_bypass_route_kind requires tgt_row == src_row + 1), which puts the two ports on different rows. The duplicated guard therefore never fires. |
| 1313 | `if kind is _BypassRoute.LEFT_ENTRY_FAMILY:` | `->L1314` |  |
| 1317 | `if kind is _BypassRoute.RIGHT_ENTRY_CROSS_ROW:` | `->L1318` |  |
| 1319 | `if kind is _BypassRoute.L_SHAPE:` | `->L1320` |  |
| 1326 | `if kind is _BypassRoute.LEFT_EXIT_AROUND_BELOW:` | `->L1327` |  |
| 1459 | `if primary_delta * run_direction.sign < 2 * MIN_STRAIGHT_EDGE + diagonal:` | `->L1460` |  |
| 1486 | `if direction_axis(transition.run_direction) is DemandAxis.X:` | `->L1490` |  |
| 1541 | `if len(candidates) != 1:` | `->L1542` |  |
| 1548 | `if direction_axis(run_direction) is DemandAxis.Y:` | `->L1549` |  |
| 1576 | `if transition is None:` | `->L1577` |  |
| 1613 | `target_crossing is None` | `->L1616` |  |
| 1614 | `or abs(source_crossing - target_crossing) > COORD_TOLERANCE` | `->L1616` |  |
| 1638 | `target_crossing is None` | `->L1641` |  |
| 1639 | `or abs(geometry.cross_lo - target_crossing) > COORD_TOLERANCE` | `->L1641` |  |
| 1651 | `if direction_axis(run_direction) is DemandAxis.Y:` | `->L1652` |  |
| 1677 | `if transition is None:` | `->L1678` |  |
| 1711 | `if direction_axis(run_direction) is DemandAxis.Y:` | `->L1712` |  |
| 1737 | `if transition is None:` | `->L1739` |  |
| 1770 | `if len(entry_group_ids) != 1:` | `->L1771` |  |
| 1777 | `if family_id is None:` | `->L1778` |  |
| 1806 | `requirement.run_direction is not None` | `->L1819` |  |
| 1807 | `and requirement.turn_direction is not None` | `->L1819` |  |
| 1808 | `and requirement.launch_coordinate is not None` | `->L1819` |  |
| 1809 | `and requirement.minimum_runway is not None` | `->L1819` |  |
| 1828 | `and requirement.turn_direction is not None` | `->L1835` |  |
| 1840 | `and family_id is not RouteFamilyId.MERGE_ENTRY` | `->L1843` |  |
| 1868 | `if any(len(runs) > 1 for runs in straight_runs_by_lane.values()):` | `->L1869` | **defensive** -- The opposed-source-run recogniser: a lane whose turn-less members disagree on the direction they leave the shared source. Every member of one exit group leaves through one exit port, whose side fixes the outward ray, and a collinear target on the other side of that port is a backward hop the engine rejects before routing. The arm names a classification that contradicts itself and declines the plan; no exit group produces it. |
| 2020 | `if any(` | `->L2031` |  |
| 2045 | `if membership is not None and membership.axis is not None:` | `->L2046` |  |
| 2123 | `if not _lane_arms_read_as_one_stroke(` | `->L2126` | **defensive** -- The reject protects a planner seed set where an unpinned cohort lands within one curve radius of a same-rank axis already claimed by a pinned cohort. Production topology planning pins or separates those cohorts before this check. test_free_lane_arm_overlapping_a_pinned_corner_uses_legacy constructs the incomplete seed set directly and covers the fallback reason. |
| 2240 | `if reason is not None:` | `->L2241` |  |
| 2343 | `if not outbound_edges or missing_connectors:` | `->L2344` |  |
| 2445 | `if final_classification.legacy_reason is not None:` | `->L2446` |  |
| 2447 | `elif tuple(seed.family_id for seed in final_classification.seeds) != tuple(` | `->L2450` |  |
| 2712 | `if incumbent.id == plan.id:` | `->L2714` |  |
| 2738 | `if prior is None:` | `->L2739` |  |
| 2782 | `if owners and plan.id not in owners:` | `->L2783` |  |
| 2783 | `for owner in owners:` | `->L2784`, `->L2785` |  |
| 2798 | `and plan.id in conflicting_plan_ids` | `->L2800` |  |
| 2816 | `and axis.fixed_anchor_offset is not None` | `->L2805` |  |
| 2817 | `and abs(proposed - axis.fixed_anchor_offset) > COORD_TOLERANCE` | `->L2819` |  |
| 3007 | `if abs(first_offset - second_offset) > COORD_TOLERANCE:` | `->L3009` |  |
| 3009 | `for owner in (first_owner, second_owner):` | `->L3006`, `->L3010` |  |
| 3010 | `if owner is not None:` | `->L3009`, `->L3011` |  |
| 3017 | `if not additions:` | `->L3019` |  |
| 3117 | `if route is None:` | `->L3118` |  |
| 3123 | `if assignment.planned_family_id is not RouteFamilyId.TB_BOTTOM_EXIT:` | `->L3124` |  |
| 3138 | `if route is None:` | `->L3139`, `->L3145` |  |
| 3153 | `if geometry.bundle_offsets is None:` | `->L3154` |  |
| 3246 | `if scaffold is None:` | `->L3247` |  |
| 3254 | `if scaffold is None:` | `->L3255` | **defensive** -- The second None check protects the manual or incomplete graph fallback after rebuilding a missing retained scaffold. Parsed render-path graphs publish one canonical scaffold during fan planning, so the fixture corpus never reaches a rebuild that can return None. Direct planner tests retain the fallback for graph construction without topology. |
| 3293 | `if overridden_dispositions:` | `->L3294` |  |
| 3295 | `for plan_id in overridden_dispositions:` | `->L3296`, `->L3307` |  |
| 3351 | `if transition_key in transition_by_edge:` | `->L3352` |  |
| 3375 | `if len(points) >= 3:` | `->L3382` |  |
| 3378 | `segment_direction(before, start) is run_direction` | `->L3382` |  |
| 3379 | `and segment_direction(start, end) is turn_direction` | `->L3382` |  |
| 3451 | `assignment is None` | `->L3456` |  |
| 3452 | `or membership.axis is None` | `->L3456` |  |
| 3453 | `or assignment.run_direction is None` | `->L3456` |  |
| 3454 | `or assignment.turn_direction is None` | `->L3456` |  |
| 3511 | `if ctx.exit_turns is None:` | `->L3512` |  |
| 3522 | `if family_id is not assignment.planned_family_id:` | `->L3523` |  |
| 3535 | `if (` | `->L3544` |  |
| 3553 | `run is None` | `->L3558` |  |
| 3554 | `or turn is None` | `->L3558` |  |
| 3555 | `or assignment.launch_coordinate is None` | `->L3558` |  |
| 3556 | `or assignment.minimum_runway is None` | `->L3558` |  |
| 3562 | `if segment_rank is None:` | `->L3563` |  |
| 3585 | `if planned_corner_offsets is None:` | `->L3586` |  |
| 3652 | `if source_turn_changed and settled is None:` | `->L3653` |  |
| 3682 | `if rank is None:` | `->L3683` |  |
| 3749 | `if current.keys() != snapshot.geometry.keys():` | `->L3750` |  |
| 3764 | `if current[key] != state:` | `->L3765` |  |
| 3797 | `station_id not in graph.stations` | `->L3800` |  |
| 3798 | `or lane.line_id not in graph.station_lines(station_id)` | `->L3800` |  |
| 3806 | `if (station_id, lane.line_id) not in station_offsets or (` | `->L3813` |  |
| 3821 | `if len(transition_routes) != 1:` | `->L3822` |  |
| 3859 | `route.exit_lane_transition_plan_id != str(exit_turn_plan.id)` | `->L3864` |  |
| 3860 | `or route.offset_regime is not OffsetRegime.BAKED` | `->L3864` |  |
| 3861 | `or route.points != expected_route.points` | `->L3864` |  |
| 3862 | `or route.curve_radii != expected_route.curve_radii` | `->L3864` |  |
| 3880 | `if not _fixed_axis_matches_plan(` | `->L3885` |  |
| 3891 | `if any(` | `->L3900` |  |
| 3908 | `if len(member_routes) != 1:` | `->L3909` |  |
| 3916 | `if route.exit_turn_family_id != assignment.planned_family_id.value:` | `->L3917` |  |
| 3923 | `if route.route_system_disposition == "compatibility":` | `->L3924` |  |
| 3926 | `if route.exit_turn_axis_id is not None:` | `->L3927` |  |
| 3935 | `if (` | `->L3949` |  |
| 3957 | `route.exit_turn_axis_id != str(assignment.axis_id)` | `->L3960` |  |
| 3958 | `or route.exit_turn_segment_rank is None` | `->L3960` |  |
| 3971 | `if (` | `->L3993` | **defensive** -- The emission validation that raises when a rendered turn's axis, lead-in runway or direction differs from the assignment the route was bound to. The emitters derive all four from that same assignment, so the raise names an emitter ignoring its plan rather than a topology. |
...
| 2482 | `if ctx.is_compatibility_edge(edge):` | `->L2483` |  |
...
76/108 gates fully exercised.

Gates with an un-exercised arm:

| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 197 | `if run_direction is None or turn_direction is None:` | `->L198` |  |
| 199 | `if (run_direction in {Direction.R, Direction.L}) == (` | `->L202` |  |
| 211 | `if not candidate_ranks:` | `->L212` |  |
| 228 | `if ctx.exit_turns is None or not pending_plan_ids:` | `->L229` |  |
| 457 | `if source is None or source is route:` | `->L459` |  |
| 500 | `if route is None:` | `->L501` |  |
| 507 | `if route is None:` | `->L508` |  |
| 540 | `if ctx.exit_turns is not None:` | `->L542` |  |
| 542 | `if ctx.convergences is not None:` | `->L544` |  |
| 718 | `if route.exit_lane_transition_plan_id is not None:` | `->L719` |  |
| 720 | `if route.fan_plan_id is not None or route.fan_route_emitter is not None:` | `->L721` |  |
| 722 | `if convergence_owns_segment_boundary(route, segment_rank):` | `->L723` |  |
| 744 | `if band is not None:` | `->L745` | **needs-review** -- _channel_bounds' reserved-band arm. The bundles that reach the feasibility search carry no realised reservation band for the channel's segment, so bounds are always taken from the gap alone. Reachable, wants a fixture whose crowded member channel also sits in a reserved band. |
| 782 | `if bounds.band is not None and not bounds.band.lo <= candidate <= bounds.band.hi:` | `->L783` | **needs-review** -- _candidate_clears_runway's band rejection, unreachable while `if band is not None:::#1` never populates a band on this path. Reachable. |
| 842 | `if abs(channel.x - target) <= COORD_TOLERANCE:` | `->L844` | **needs-review** -- _align_same_line_channels' re-seat arm. Its one fixture (genomeassembly_organellar) still reaches the gate but now finds the channel already on its carrier's claim coordinate, so the reseat is skipped. Bisecting the range shows 0f58d60c -- ranking member channels over a gap's whole population -- is what makes the claim and the channel coincide on arrival, not the bundle-pitch packing. Reachable; the frozen render corpus means no fixture was authored. |
| 867 | `if coordinate is not None:` | `->L822`, `->L868` |  |
| 939 | `if overlap <= MIN_CORRIDOR_Y_OVERLAP:` | `->L940` |  |
| 1033 | `if item.candidate.route.line_id in claim.line_ids and (` | `->L1036` | **needs-review** -- feasible()'s same-line-carrier clause, which forbids a candidate delta that would split a carrier the bundle already shares with a claim. No fixture reaches the feasibility search with a same-line source-compatible claim in range: _align_same_line_channels seats such a member on its claim before the bundle allocation runs. Reachable, wants a fixture. |
| 1036 | `if abs(coordinate - claim.coordinate) > COORD_TOLERANCE:` | `->L1037`, `->L1038` | **needs-review** -- The rejection inside feasible()'s same-line-carrier clause. Unreached for the same reason as its enclosing gate `if item.candidate.route.line_id in claim.line_ids and (:::#1`. Reachable. |
| 1176 | `if radii is not None:` | `->L1181` |  |
| 1177 | `if 0 <= rank - 1 < len(radii):` | `->L1179` |  |
| 1179 | `if rank < len(radii):` | `->L1181` |  |
| 1211 | `elif abs(start[0] - end[0]) <= COORD_TOLERANCE:` | `->L1201` |  |
| 1279 | `if movable_run:` | `->L1280` |  |
| 1324 | `if context is not None:` | `->L1326` | **defensive** -- _convergence_context_route returns None only when ctx.edge_by_key has no entry for the key, and edge_by_key is built from every graph.edge while the key comes from scaffold.edge_order, which is derived from the same edges. Same never-taken lookup as the sibling `if edge is None` guard on the non-convergence path (0 of 141 fixtures). |
| 1327 | `if key in ctx.skip_edges:` | `->L1328` | **candidate-dead** -- Subsumed by the convergence-edge check two lines above. Instrumenting all 348 fixtures: ctx.skip_edges holds exactly one merge-junction->entry-port hop, it is always present in scaffold.edge_order, and it is always also a convergence member edge, so the loop continues at the convergence arm and never tests skip_edges (0 of 141 fixtures). Proposed simplification: drop the two lines. Not applied here -- deletion is a deliberate separate pass, and a skip hop that is not convergence-owned is not ruled out by contract. |
| 1330 | `if edge is None:` | `->L1331` |  |
| 1334 | `if family_id is None:` | `->L1335` |  |
| 1356 | `if system_id not in failures:` | `->L1360` |  |
| 1517 | `or route.route_system_disposition != RouteSystemDisposition.PLANNED.value` | `->L1519` | **needs-review** -- validate_member_geometry_emission' skip for a route that holds a frozen member geometry plan while its route system did not end up planned. build_member_geometry_execution drops every candidate of a system that fails, so a plan-holding route can only be non-planned when the system is escalated after the freeze -- which happens exactly when a convergence conflict escalates it to the compatibility disposition. No corpus system carries such a conflict, so the operand never decides. Reachable through the same fixture that would close any of this module's conflict arms. |
| 1520 | `if tuple(route.route_system_owned_segment_ranks) != plan.owned_segment_ranks:` | `->L1521` |  |
| 1526 | `if actual != (channel.start, channel.end):` | `->L1527` |  |

## `normalize.py`

322/456 gates fully exercised.
...
| 732 | `if gap_right <= gap_left:` | `->L733` |  |
| 768 | `if ctx.exit_turns is None:` | `->L769` |  |
| 791 | `if planned_offsets is None:` | `->L792` |  |
| 811 | `dx is None` | `->L815` |  |
| 813 | `or not 0 <= radius_index < len(route.curve_radii)` | `->L815` |  |
| 820 | `if (` | `->L824` |  |
| 855 | `if gap_right <= gap_left:` | `->L856` |  |
| 999 | `go is not None` | `->L1005` |  |
| 1000 | `and gt is not None` | `->L1005` |  |
| 1037 | `if len(pts) < 2:` | `->L1038` |  |
| 1053 | `if abs(y2 - y1) > COORD_TOLERANCE or abs(x2 - x1) <= COORD_TOLERANCE:` | `->L1054` |  |
| 1130 | `if slot is None or trunk is None or trunk[0] != k:` | `->L1131` | **needs-review** -- _reconcile_moved_trunk_slot's precondition guard: the re-banded leg is not the route's first interior horizontal run, or the route declared no trunk slot. The docstring states this as a real filter ('only the trunk the slot describes can retarget it'), and _bundle_divergent_distinct_traverses hands it every distinct-line traverse leg, so a non-trunk leg or a slotless route reaches it; all 3 corpus fixtures happen to pass. Reachable; wants a fixture. |
| 1141 | `if any(abs(channel.x - ref_x) > COORD_TOLERANCE for channel in planned[1:]):` | `->L1142` |  |
| 1142 | `if ctx.validate_final_route_frames:` | `->L1143`, `->L1144` |  |
| 1188 | `if channel is None:` | `->L1189` |  |
| 1261 | `if planned and max(planned) - min(planned) > COORD_TOLERANCE:` | `->L1262` |  |
| 1267 | `if ref is not None:` | `->L1268` |  |
| 1334 | `for group in _merge_feeder_groups(routes, ctx):` | `->L1337` |  |
| 1343 | `if compatibility_channels:` | `->L1334`, `->L1344` |  |
| 1381 | `if not downs or not ups:` | `->L1383` |  |
| 1384 | `for descent in downs:` | `->L-1350`, `->L1385` |  |
| 1386 | `if col in moved:` | `->L1387`, `->L1388` |  |
| 1389 | `for up in ups:` | `->L1390`, `->L1399` |  |
| 1390 | `if up.route.line_id != descent.route.line_id:` | `->L1391`, `->L1392` | **defensive** -- _clear_merge_trunk_opposite_arm skips an up-arm carrying a different line from the down-trunk, since only a same-line opposite arm folds the line back over itself. The corpus's one fork sends a single line both ways, so the different-line skip arm stays un-exercised. Defensive against a fork whose opposite arms carry distinct lines. |
| 1392 | `if abs(up.x - descent.x) > radius:` | `->L1393`, `->L1394` |  |
| 1395 | `if overlap <= COORD_TOLERANCE:` | `->L1396`, `->L1397` | **defensive** -- _clear_merge_trunk_opposite_arm skips an opposite arm within a curve radius of the down-trunk but not overlapping it in Y, since without a shared Y span there is no fold-back to clear. The corpus's qualifying fork overlaps in Y, so the no-overlap skip arm stays un-exercised. Defensive against a near-column opposite arm that clears the trunk vertically. |
| 1399 | `if target_x is None:` | `->L1400`, `->L1401` |  |
| 1402 | `for ch in downs:` | `->L1384`, `->L1403` |  |
| 1403 | `if abs(ch.x - descent.x) <= COORD_TOLERANCE:` | `->L1402`, `->L1404` | **defensive** -- _clear_merge_trunk_opposite_arm moves every down-leg sharing the folding descent's column and skips those on other columns. The corpus's one folding descent carries all its down-legs on a single shared column, so the other-column skip arm stays un-exercised. Defensive against a fork with down-legs spread over several columns. |
| 1430 | `if k == desc.idx + 1:` | `->L1429` | **defensive** -- In _fanout_traverse_spans: the corpus fan-out routes carry the traverse as their only interior horizontal trunk, so the loop matches on the first trunk and the non-match arm is never taken. Defensive. |
| 1474 | `if lower > upper + COORD_TOLERANCE_FINE:` | `->L1475` |  |
| 1486 | `if any(` | `->L1496` |  |
| 1522 | `abs(p0[1] - p1[1]) > COORD_TOLERANCE` | `->L1526` |  |
| 1523 | `or abs(p1[0] - p2[0]) > COORD_TOLERANCE` | `->L1526` |  |
| 1524 | `or abs(p2[1] - p3[1]) > COORD_TOLERANCE` | `->L1526` |  |
| 1543 | `if source_col is None:` | `->L1544` |  |
| 1559 | `if route_system_owns_segment_boundary(route, opening.idx):` | `->L1560` |  |
| 1576 | `if not sibling_xs:` | `->L1577` |  |
| 1579 | `if new_x <= max(sibling_xs) + ctx.curve_radius + COORD_TOLERANCE:` | `->L1580` |  |
| 1582 | `if _section_intrudes(` | `->L1589` |  |
| 1609 | `if any(` | `->L1613` |  |
| 1627 | `if len(turns) != 1:` | `->L1628` | **defensive** -- In _bundle_divergent_distinct_traverses: every fan in the corpus turns its traverses the same way, so the mixed-direction skip is never taken. Defensive. |
| 1648 | `if any(` | `->L1652` | **defensive** -- Compatibility convergence may not move a planner-owned final channel or reseat a channel through a foreign section. Complete direct-port opposing tails are unplanned and placement reserves their target band; the true arm therefore defends broken ownership or obstruction state. test_planner_owned_opposing_group_is_left_atomic and test_obstructed_opposing_group_is_left_atomic exercise both rejection reasons. |
| 1691 | `elif ctx.merge.entry_port_for.get(rp.edge.source) == rp.edge.target:` | `->L1692` |  |
| 1702 | `for merge_id, hop in hop_by_merge.items():` | `->L1703` |  |
| 1704 | `if not feeders or any(ends_at(route, hop.points[0]) for route in feeders):` | `->L1705`, `->L1706` |  |
| 1709 | `if carrier is None:` | `->L1710`, `->L1711` | **defensive** -- A removable merge-entry hop has feeders, none ending at the hop start, and one ending at the hop destination after feeder normalization. A missing carrier therefore signals broken settled geometry; the guard keeps the hop instead of discarding a visible route. |
| 1718 | `if not covered:` | `->L1720` |  |
| 1778 | `for _ in range(max(1, len(buckets))):` | `->L-1724` | **defensive** -- Bounded fixed-point repair for coincident corner radii. Production routes settle without a second correction pass; the multi-bucket propagation and loop-exhaustion paths are exercised directly by test_unify_refreshes_a_shared_route_after_changing_one_corner. |
| 1796 | `if actual - common <= COORD_TOLERANCE_FINE:` | `->L1798` | **defensive** -- Production coincident turns have enough shared runway after planning and already resolve to the same radius. The correction arm is a defensive repair for directly supplied or future route geometry and is exercised by the coincident-corner unit tests. |
| 1801 | `for _step in range(64):` | `->L1802`, `->L1809` | **defensive** -- Fixed-iteration bisection used only when a defensive coincident-radius repair must lower one route. The correction and completed bisection are exercised by test_unify_uses_widest_radius_all_shared_legs_can_resolve. |
| 1805 | `if resolved < common:` | `->L1806`, `->L1808` | **defensive** -- Both bisection directions belong to the defensive coincident-radius repair rather than ordinary topology dispatch. The short-runway unit case exercises the search and verifies the final common effective radius. |
| 1816 | `if not changed:` | `->L1778` | **defensive** -- The production corpus reaches the immediate fixed point because coincident members already share an effective radius. The continue-for-propagation arm is a defensive repair path covered by test_unify_refreshes_a_shared_route_after_changing_one_corner. |
| 1851 | `if _planner_owns_channel(ch):` | `->L1852` |  |
| 1915 | `if opposing_bundle is not None and {` | `->L1918` | **defensive** -- iter_opposing_entry_confluences requires the complete direct-port line set, while by_port can only add a member through convergence-junction aliasing. Resolved convergence junctions replace the direct approach rather than coexist with a complete direct-port bundle, so unequal ownership sets require an inconsistent route/context pair. test_opposing_group_with_an_extra_mapped_approach_uses_generic_ownership exercises the defensive fallback. |
| 1925 | `if any(` | `->L1934` |  |
| 2006 | `if any(` | `->L2011` |  |
| 2047 | `if has_planned_channel and any(` | `->L2052` |  |
| 2103 | `if any(` | `->L2107` |  |
| 2108 | `if not crossing:` | `->L2115` |  |
| 2118 | `if abs(delta) <= COORD_TOLERANCE:` | `->L2119`, `->L2123` | **defensive** -- _snap_merge_feeder_group skips a merge feeder whose opening descent already sits on the trunk's shared column (no shift needed). The corpus's merge feeders all reach the trunk from a distinct column, so the already-coincident skip arm stays un-exercised. Defensive and idempotent across the routing replays a render performs. |
| 2123 | `for r, k, _y in crossing:` | `->L2080`, `->L2124` |  |
| 2245 | `if rp.curve_radii is None:` | `->L2246` | **defensive** -- curve_radii is always a concrete list for inter-section routes (every inter-section handler sets it); the None arm in _reseat_concentric_flanking is a belt-and-suspenders guard. |
| 2453 | `if route.curve_radii is None:` | `->L2454` | **defensive** -- Fan-opening channels are selected from a real horizontal-to-vertical turn, and production turn builders attach radius metadata to that shape. The None arm protects a synthetic or incomplete RoutedPath. |
| 2520 | `if port is not None and port.is_entry:` | `->L2522` |  |
| 2544 | `if (` | `->L2551` |  |
| 2563 | `if any(` | `->L2567` |  |
| 2675 | `if not trunks:` | `->L2676`, `->L2677` | **defensive** -- Shape guard in _merge_convergence_run: a trunk routed as a U-shaped bypass always has an interior horizontal leg for iter_horizontal_trunks to find, so the corpus never takes the None arm. Not provably unreachable across every branch of _route_bypass, so it stays a guard rather than an assert (#1597). |
| 2702 | `if not verticals:` | `->L2703` | **defensive** -- Shape guard in _land_feeder_on_run: a merge branch feeder is built as a descent into a tail, so it always has a vertical leg. Reachable in principle only if a feeder's source Y coincides with the trunk channel level, collapsing the descent below COORD_TOLERANCE (#1597). |
| 2707 | `if k + 2 != len(pts) - 1 or y_hi - y_lo < radius:` | `->L2708` | **defensive** -- Shape guard in _land_feeder_on_run. _route_merge_branch emits exactly four waypoints, while an alternate attached-source construction or a malformed hand-built route can add points; the second half rejects a descent too short to re-form its corner. Neither shape occurs in the corpus (#1597). |
| 2718 | `if rp.curve_radii is not None:` | `->L2720` | **defensive** -- Truncating the tail in _land_feeder_on_run keeps curve_radii aligned with the shortened waypoint list when metadata is present. Hand-built and compatibility routes may omit that metadata, so the field cannot be treated as always present (#1597). |
| 2746 | `if len(rp.points) != 2:` | `->L2747`, `->L2748` |  |
| 2749 | `if abs(lane - _get_offset(ctx, rp.edge.target, rp.line_id)) <= COORD_TOLERANCE:` | `->L2750`, `->L2751` |  |
| 2756 | `if not (riser_spans_lane and riser_ahead):` | `->L2757`, `->L2758` | **defensive** -- Reachability guard in _land_lane_changing_feeder_on_trunk_riser: the trunk's riser has to cross the feeder's lane and stand in the feeder's direction of travel before the feeder can terminate on it. Only one corpus fixture reaches the guard at all (exit_run_three_drop_columns, the sole adjacent merge feeder whose direct run changes lane), and there the riser both spans the lane and stands ahead, so the reject arm needs a trunk that turns before the feeder junction or lands short of its lane. |
| 2780 | `if trunk_rp.convergence_plan_id is not None:` | `->L2782` |  |
| 2783 | `if run is None:` | `->L2784`, `->L2785` | **defensive** -- Consumer of the _merge_convergence_run None arm above; unreachable for exactly the same reason and kept for the same reason (#1597). |
| 2785 | `for rp in others:` | `->L2779`, `->L2786` |  |
| 2786 | `if (rp.edge.source, rp.edge.target, rp.line_id) in merge.branch_edges:` | `->L2787`, `->L2789` |  |
...
| 3393 | `if band is not None:` | `->L3399` | **defensive** -- Same-line dogleg: the band-found arm is exercised by dogleg_exempt_sameline; the band-is-None arm is defensive (both colliding trunks route through _center_inter_row_channel, so the trunk Y always resolves to a gap band). |
| 3414 | `if down_ok and up_ok and (cross_down is None) != (cross_up is None):` | `->L3416` | **defensive** -- Same-line dogleg side selection. The crossing-decided true arm (->1297, pick the side whose riser does not pierce the exempt run) is exercised by dogleg_exempt_sameline. The false fall-through (->1298) needs either an out-of-band side or a crossing-equal tie, neither reachable by a clean topology: the reachable dogleg regime rigidly fixes the inter-row band so down_room/up_room stay >= min_sep=6 (observed 14/26 across the corpus and every constructed variant; perturbing the grid loses the regime), and the only realizable tie is both-cross (a crossing-free tie needs degenerate zero-length exempt risers) which co-occurs with section-crossing / dogleg-crosses-exempt guard failures and is never a shippable render. |
| 3416 | `elif down_ok and (not up_ok or prefer_down):` | `->L3417`, `->L3418` | **defensive** -- Same-line dogleg tie / out-of-band fallback, reached only after the L1296 if is false, which a clean topology never produces (see that gate): the true arm (->1299, lean down on a tie) needs a crossing-free tie that requires degenerate risers, and the false arm (->1300) needs down_ok False, which the regime's fixed inter-row band (down_room >= min_sep) never yields. |
| 3418 | `elif up_ok:` | `->L3419`, `->L3421` | **defensive** -- Same-line dogleg move-up / give-up fallback, reached only when both L1296 and L1298 are false. The true arm (->1301, move up because the lower gap is too tight) needs down_ok False and the else arm (->1303, leave fused) needs both rooms below min_sep; the reachable regime's inter-row band is auto-widened so both rooms stay >= min_sep=6 (observed 14/26), so neither is reached by a valid topology. |
| 3446 | `if band is not None:` | `->L3451` | **defensive** -- Distinct-line dogleg: the band-found arm (->1327) is exercised by dogleg_exempt_distinct (#702). The remaining band-is-None arm (->1331) is defensive: a trunk reaching this second loop collides with an exempt bypass obstacle that lives in an inter-row gap, so _inter_row_gap_band(ctx, t.y) always resolves to a band - the same mechanism already accepted for the sibling 'if band is not None:::#1'. |
| 3461 | `if below_ok and above_ok and (cross_below is None) != (cross_above is None):` | `->L3463` | **defensive** -- Distinct-line dogleg side selection. The crossing-decided true arm (->1342) is exercised by dogleg_exempt_distinct. The false fall-through (->1343) needs a crossing-equal tie; a crossing-free tie requires degenerate zero-length exempt risers and the only realizable tie is both-cross, which trips _guard_no_dogleg_crosses_exempt_trunk plus the section-crossing guards and is never a shippable render. |
| 3463 | `elif below_ok and (not above_ok or prefer_below):` | `->L3464`, `->L3465` | **defensive** -- Distinct-line dogleg tie fallback, reached only after L1341 is false (a tie), which a clean topology never produces (see that gate). The true arm (->1344, lean below) would fire on such a tie; the false arm (->1345) is unreachable because below_ok always holds and prefer_below is always true (t.y == hit.y in this regime). |
| 3465 | `elif above_ok:` | `->L3466`, `->L3468` | **defensive** -- Distinct-line dogleg move-up/skip branch: unreachable because the below branch is always taken (t.y == hit.y, below_ok true); covers both the move-up and the gap-too-tight continue. |
| 3476 | `for section in graph.sections.values():` | `->L3477`, `->L3488` |  |
| 3483 | `held[0] - COORD_TOLERANCE < coord < held[1] + COORD_TOLERANCE` | `->L3476`, `->L3484` | **defensive** -- Held-axis bracket in _run_enters_section, the section-intrusion veto's first test. No corpus separation target lands within a section's extent on the held axis, so the inside arm never fires; the same defensive reasoning as the veto it serves. |
| 3484 | `and span[0] < crossed[1]` | `->L3476`, `->L3485` | **defensive** -- Crossed-axis overlap test in _run_enters_section, reached only for a target already bracketed on the held axis. No corpus separation target is, so this operand's short-circuit arm never fires. |
| 3485 | `and crossed[0] < span[1]` | `->L3476`, `->L3487` | **defensive** -- Second half of the crossed-axis overlap test in _run_enters_section, reached only once the first half holds. No corpus separation target reaches it, so its short-circuit arm never fires. |
| 3498 | `for run in lane.runs:` | `->L3499`, `->L3508` |  |
| 3554 | `if i in relocated:` | `->L3555` | **defensive** -- Re-entry skip in the co-travelling separation worklist. A track is re-queued only while it is still un-relocated, so the second copy of a track queued twice before its first pop is the only way to reach the skip; across the corpus every re-queued track is popped before it moves, so the taken arm never fires. Defensive against a crowded stack whose neighbours re-queue one track twice, where taking it twice would break the one-move-per-track bound. |
| 3562 | `if obstacle is None or abs(lane.coord - obstacle.coord) <= COORD_TOLERANCE_FINE:` | `->L3564` |  |
| 3565 | `if any(` | `->L3569`, `->L3570` | **needs-review** -- Never re-band a fan traverse across a section it does not belong to: the re-band would draw the traverse through an unrelated box. The corpus reached the crossing arm only through merge_trunk_over_low_section's pre-fix geometry, where a group carrying a member-geometry-owned channel was skipped and its traverses kept coordinates that crossed a foreign section. With the group gated on the coordinates a plan resolves outright, no fixture crosses one, so the guard keeps a real hazard with no corpus witness. |
| 3611 | `if sg[0].sign_x != t.sign_x:` | `->L3612` | **defensive** -- Both callers of _coincident_trunk_slots pre-filter input to a single traversal direction (band = [t for t in grp if t.sign_x == sign]), so within the function sg[0].sign_x != t.sign_x is always False; the opposing-direction continue is unreachable. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3642 | `if rp.curve_radii is None:` | `->L3643` | **defensive** -- curve_radii is always set for inter-section routes; the None arm in _restack_htrunk is a defensive guard. |
| 3654 | `if 0 <= k - 1 < len(rp.curve_radii):` | `->L3656` | **defensive** -- Bounds guard in _restack_htrunk: trunk index k >= 1 so k-1 >= 0 is always in range; the skip arm is defensive. |
| 3656 | `if k < len(rp.curve_radii) and k + 2 < len(pts):` | `->L-3621` | **defensive** -- Bounds guard in _restack_htrunk: for a real interior trunk k and k+2 are in range; the skip arm is defensive. |
| 3715 | `if down is None or len(up.points) < 2:` | `->L3716` | **needs-review** -- Un-exercised on origin/main too; 897747b0 covered it on 13 fixtures and HEAD is back to main's [0,138] split, so this is a return to main's behaviour rather than a new gap. Reachable by construction (main's baseline lists it as an open gap already). No fixture authored -- the render corpus is frozen for a measurement against main. |
| 3761 | `if len(targets) != 1:` | `->L3762` |  |
| 3771 | `if col is None:` | `->L3772` | **defensive** -- Defensive guard in _convergence_line_order: a peel-off riser channel always resolves to a source-section column, so the unresolved-column early return is never taken on the corpus. |
| 3812 | `if not 0 <= far < len(pts):` | `->L3813` |  |
| 3815 | `if abs(dx) > COORD_TOLERANCE:` | `->L3809` |  |
| 3817 | `if not votes:` | `->L3818` |  |
| 3921 | `if ca != cb:` | `->L3923` |  |
| 3923 | `if rep_x[a] != rep_x[b]:` | `->L3924`, `->L3925` | **defensive** -- The routing system assigns distinct x offsets to every line sharing a column gap, so two distinct lines in one gap-bundle corridor never share a representative x; the equal-rep_x tiebreak fall-through in _distinct_line_order is unreachable (exhaustive corpus scan confirms). Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3953 | `if rp.curve_radii is None:` | `->L3954` | **defensive** -- curve_radii is always set for inter-section routes; the None arm in _restack_channel is a defensive guard. |
| 3956 | `if not 0 <= radius_idx < len(rp.curve_radii):` | `->L3957` |  |
| 3980 | `if abs(ly - pts[1][1]) < COORD_TOLERANCE:` | `->L-3930` | **defensive** -- A re-stacked gap channel at points-index 1 always has a horizontal lead-in (L-shape and bypass routes open with a horizontal segment); routes opening non-horizontally are normalize_exempt and skipped by the materialization. The non-horizontal-lead return in _restack_channel is unreachable. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 4010 | `if anchor_section_id is not None and anchor_side is not None:` | `->L4012` | **defensive** -- _gap_channel_base falls back to the column gap when passed a null anchor. All six call sites (in _route_bypass) pass the bypass source/target section id, which is None only when that endpoint has no resolvable section (src_sec_id/tgt_sec_id in _route_bypass); the corpus has no bypass with a sectionless endpoint, so the null-anchor arm is never taken. Defensive guard for a sectionless bypass endpoint. |
| 4065 | `elif right_ok:` | `->L4068` |  |
| 4114 | `if s.bbox_w <= 0:` | `->L4115` | **defensive** -- Degenerate-bbox guard in _v_segment_crosses_other_section: mirror of the _h_segment_penetrates_section guard; sections always have positive width. |
| 4494 | `if not blockers:` | `->L4495` |  |
