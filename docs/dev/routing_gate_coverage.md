...
**2239/3178** gates fully exercised (both/all arms hit by some fixture); **939** gates have at least one un-exercised arm.
...
| 4867 | `if direction is None:` | `->L4868` |  |
| 4896 | `if len(points) < 3:` | `->L4897` |  |
| 4948 | `if not section_ids:` | `->L4949` |  |
| 5028 | `if mid_y is None:` | `->L5045` |  |
| 5060 | `channel_y is None` | `->L5090` |  |
| 5062 | `and src_sec is not None` | `->L5090` | **defensive** -- Inter-row exit mid-Y clearance bump (the n>1 multi-line branch). When more than one line shares the exit the source section always resolves, so the `src_sec is None` short-circuit past the clearance bump is never taken. |
| 5063 | `and tgt_sec is not None` | `->L5090` | **defensive** -- Companion to src_sec in the same n>1 mid-Y clearance branch: the target section always resolves there, so the `tgt_sec is None` short-circuit past the bump is never taken. |
| 5092 | `if exit_side is not None:` | `->L5094` |  |
| 5094 | `elif abs(dx) > ctx.curve_radius:` | `->L5095`, `->L5097` |  |
| 5098 | `if src.id in ctx.graph.junctions:` | `->L5099`, `->L5111` | **needs-review** -- Reachable only via a defective render (LR->TB TOP-entry routing); see #720. |
| 5099 | `for je in ctx.graph.edges_to(src.id):` | `->L5100`, `->L5111` |  |
| 5101 | `if js.is_port:` | `->L5099`, `->L5102` |  |
| 5102 | `if abs(js.x - src.x) <= COORD_TOLERANCE:` | `->L5103`, `->L5105` |  |
| 5117 | `if exit_side is not None and not straight_drop:` | `->L5127` |  |
| 5165 | `if not straight_drop:` | `->L5169` | **needs-review** -- A top-entry fan branch fed straight from directly above the junction (straight_drop) keeps its lead-in column at the source X rather than the shared fan pivot; the corpus's fan branches all carry horizontal travel, so the straight-drop arm is unexercised. |
| 5167 | `if lead is Direction.R:` | `->L5169` | **needs-review** -- A top-entry fan branch leading LEFT out of the junction skips the _v1_corner_x right-edge clearance bump; the corpus's fan branches all lead right, so the left-lead arm is unexercised. |
| 5194 | `if straight_drop:` | `->L5195` |  |
| 5271 | `if ctx.exit_turns is None:` | `->L5272` | **needs-review** -- _perp_entry_turn_is_planned answers False for the pre-plan context the exit-turn planner probes emitters with; ctx.exit_turns is bound only for emission. That context does reach these emitters -- the same guard in _seat_bypass_descent takes its None arm on packed_cell_right_exit_left_entry_wrap -- but no corpus probe reaches the TOP/BOTTOM-entry L-shape records this predicate feeds. |
| 5356 | `if src_sec is None or tgt_sec is None or src_sec.grid_row >= tgt_sec.grid_row:` | `->L5359` |  |
| 5526 | `if src_section and src_section.bbox_w > 0:` | `->L5527` |  |
| 5547 | `if tgt_sec is None:` | `->L5548` |  |
| 5618 | `if src_col is None:` | `->L5619` |  |
| 5795 | `if run_direction is None:` | `->L5800` |  |
| 5857 | `if src_sec is not None:` | `->L5888` | **defensive** -- Defensive guard in the fanned entry-wrap source-clearance lift: a fan-out junction always resolves to its source section, so the None arm is unreachable in practice. |
| 5894 | `if tgt_col is not None:` | `->L5899` | **defensive** -- tgt_col from _resolve_section_col on a valid LEFT entry port is always non-None (sections get grid_col>=0 after auto-layout). |
| 5896 | `if shared_vx is not None:` | `->L5899` | **defensive** -- _fan_left_entry_descent_x returns None only when col_left<=0; col_left_edge for the target's own column is always >0. |
| 6022 | `if entry_side is PortSide.LEFT:` | `->L6025` |  |
| 6090 | `if entry_port is None:` | `->L6091` | **defensive** -- _route_around_section_below is always called with a non-None entry_port (both dispatch sites pass a checked station). |
| 6101 | `if trunk_src is None or trunk_src == edge.source:` | `->L6103` | **needs-review** -- Reachable only via a defective render (merge-junction tangle); see #721. |
| 6154 | `if col_left <= 0.0:` | `->L6155` | **defensive** -- col_left_edge's 0.0 default only fires for a column with no sections; the target's own column always holds its section. |
| 6191 | `if entry_port is None:` | `->L6192` | **defensive** -- Every caller of _corridor_is_viable passes a non-None entry_port. |
| 6198 | `if src_row is None or ep_row is None or src_col is None or ep_col is None:` | `->L6199` | **defensive** -- _resolve_section_colrow returns (None,None) only for a station with no section; valid entry ports always have a section with non-negative grid coords. |
| 6285 | `if fan is not None and corridor is not None and corridor.band_y is not None:` | `->L6289` | **needs-review** -- Corridor-feeder shared inter-row band gate: the corpus's corridor feeders belong to fans whose corridor carries a band_y, so the short-circuit past it (no corridor or bypass-only) is unexercised; it falls to the elif fan-is-not-None global-band branch. |
| 6289 | `elif fan is not None:` | `->L6293`, `->L6298` | **needs-review** -- Corridor-feeder fallback for a fan whose junction earned no corridor (its in-column gap below does not fit the bundle); the corpus's fans all earn a corridor, so this global-band branch is unexercised. |
| 6298 | `elif gap_bottom > gap_top:` | `->L6299`, `->L6303` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6308 | `if fan is None and gap_bottom > gap_top:` | `->L6309` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6322 | `if fan is not None and ep_col is not None:` | `->L6324` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6324 | `if vx is None:` | `->L6325` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6426 | `if ep_section and ep_section.bbox_w > 0:` | `->L6429` | **defensive** -- entry_port.section_id is always set and the section has bbox_w>0 after layout; the else arm never fires. |
| 6723 | `for claim in ctx.convergences.prior_channel_claims_for_edge(edge):` | `->L6724` |  |
| 6724 | `if claim.line_id != edge.line_id or claim.owner_source == edge.source:` | `->L6725`, `->L6726` |  |
| 6726 | `if not (corner_x - COORD_TOLERANCE <= claim.x <= gap_right + COORD_TOLERANCE):` | `->L6727`, `->L6728` |  |
| 6728 | `if min(hi, claim.y_hi) - max(lo, claim.y_lo) > COORD_TOLERANCE:` | `->L6723`, `->L6729` |  |
| 6802 | `if _leadout_self_meets_sibling_descent(ctx, edge, corner_x, f.sy, hy, gap_right):` | `->L6803` |  |
| 6934 | `if gap_bottom <= gap_top:` | `->L6935` | **defensive** -- The row-gap enforcer keeps stacked rows separated, so the inter-row band above a target row (row_bottom_edge(tgt_row-1) up to row_top_edge(tgt_row)) has positive height whenever a RIGHT entry is fed from a higher row; the degenerate-band arm guards an abutting-rows layout placement never produces. |
| 6938 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L6939` | **defensive** -- Band-fit test in _right_entry_gap_above_is_clear (#889), reached only when the cross-row RIGHT-entry drop-in is not clear (the exotic blocked-descent case above). The corpus always takes the drop-in, so neither arm of the gap-above fallback is exercised. Defensive. |
| 7093 | `if gap_bottom <= gap_top:` | `->L7094` |  |
| 7095 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L7096` |  |
| 7106 | `if f.v_segment_crosses_other_section(vx, gy, tgt.y):` | `->L7107` |  |
| 7201 | `if source is None or source.bbox_w <= 0:` | `->L7202` |  |
| 7206 | `if source.id not in members:` | `->L7207` |  |
| 7212 | `if other.bbox_x <= source_right + COORD_TOLERANCE:` | `->L7213` |  |
| 7214 | `if cellmate_left is None or other.bbox_x < cellmate_left:` | `->L7208` |  |
| 7216 | `if cellmate_left is None:` | `->L7217` |  |
| 7254 | `if band0_bottom <= band0_top or not _inter_row_band_fits(band0_top, band0_bottom):` | `->L7255` |  |
| 7256 | `if band1_bottom <= band1_top or not _inter_row_band_fits(band1_top, band1_bottom):` | `->L7257` |  |
| 7258 | `if band1_top <= band0_bottom + COORD_TOLERANCE:` | `->L7259` |  |
| 7274 | `if f.v_segment_crosses_other_section(corner_x, band0_y, band1_y, exclude):` | `->L7275` |  |
| 7331 | `if geom is None:` | `->L7332` |  |
| 7334 | `if f.v_segment_crosses_other_section(geom.lead_x, src.y, geom.band0_y):` | `->L7335` |  |
| 7336 | `if f.h_segment_crosses_other_section(geom.lead_x, geom.corner_x, geom.band0_y):` | `->L7337` |  |
| 7338 | `if f.h_segment_crosses_other_section(geom.corner_x, geom.vx, geom.band1_y):` | `->L7339` |  |
