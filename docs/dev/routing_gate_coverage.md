...
**1946/2823** gates fully exercised (both/all arms hit by some fixture); **877** gates have at least one un-exercised arm.
...
185/288 gates fully exercised.

Gates with an un-exercised arm:

| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 490 | `if not replacements:` | `->L491` |  |
...
| 1805 | `if (` | `->L1825` |  |
| 1834 | `and requirement.turn_direction is not None` | `->L1841` |  |
| 1846 | `and family_id is not RouteFamilyId.MERGE_ENTRY` | `->L1849` |  |
| 1874 | `if any(len(runs) > 1 for runs in straight_runs_by_lane.values()):` | `->L1875` | **defensive** -- The opposed-source-run recogniser: a lane whose turn-less members disagree on the direction they leave the shared source. Every member of one exit group leaves through one exit port, whose side fixes the outward ray, and a collinear target on the other side of that port is a backward hop the engine rejects before routing. The arm names a classification that contradicts itself and declines the plan; no exit group produces it. |
| 2026 | `if any(` | `->L2037` |  |
| 2125 | `if not _lane_arms_read_as_one_stroke(` | `->L2128` | **defensive** -- The reject protects a planner seed set where an unpinned cohort lands within one curve radius of a same-rank axis already claimed by a pinned cohort. Production topology planning pins or separates those cohorts before this check. test_free_lane_arm_overlapping_a_pinned_corner_uses_legacy constructs the incomplete seed set directly and covers the fallback reason. |
| 2242 | `if reason is not None:` | `->L2243` |  |
| 2345 | `if not outbound_edges or missing_connectors:` | `->L2346` |  |
| 2447 | `if final_classification.legacy_reason is not None:` | `->L2448` |  |
| 2449 | `elif tuple(seed.family_id for seed in final_classification.seeds) != tuple(` | `->L2452` |  |
| 2714 | `if incumbent.id == plan.id:` | `->L2716` |  |
| 2740 | `if prior is None:` | `->L2741` |  |
| 2784 | `if owners and plan.id not in owners:` | `->L2785` |  |
| 2785 | `for owner in owners:` | `->L2786`, `->L2787` |  |
| 2800 | `and plan.id in conflicting_plan_ids` | `->L2802` |  |
| 2818 | `and axis.fixed_anchor_offset is not None` | `->L2807` |  |
| 2819 | `and abs(proposed - axis.fixed_anchor_offset) > COORD_TOLERANCE` | `->L2821` |  |
| 3009 | `if abs(first_offset - second_offset) > COORD_TOLERANCE:` | `->L3011` |  |
| 3011 | `for owner in (first_owner, second_owner):` | `->L3008`, `->L3012` |  |
| 3012 | `if owner is not None:` | `->L3011`, `->L3013` |  |
| 3019 | `if not additions:` | `->L3021` |  |
| 3119 | `if route is None:` | `->L3120` |  |
| 3125 | `if assignment.planned_family_id is not RouteFamilyId.TB_BOTTOM_EXIT:` | `->L3126` |  |
| 3140 | `if route is None:` | `->L3141`, `->L3147` |  |
| 3155 | `if geometry.bundle_offsets is None:` | `->L3156` |  |
| 3248 | `if scaffold is None:` | `->L3249` |  |
| 3256 | `if scaffold is None:` | `->L3257` | **defensive** -- The second None check protects the manual or incomplete graph fallback after rebuilding a missing retained scaffold. Parsed render-path graphs publish one canonical scaffold during fan planning, so the fixture corpus never reaches a rebuild that can return None. Direct planner tests retain the fallback for graph construction without topology. |
| 3295 | `if overridden_dispositions:` | `->L3296` |  |
| 3297 | `for plan_id in overridden_dispositions:` | `->L3298`, `->L3309` |  |
| 3353 | `if transition_key in transition_by_edge:` | `->L3354` |  |
| 3377 | `if len(points) >= 3:` | `->L3384` |  |
| 3380 | `segment_direction(before, start) is run_direction` | `->L3384` |  |
| 3381 | `and segment_direction(start, end) is turn_direction` | `->L3384` |  |
| 3453 | `assignment is None` | `->L3458` |  |
| 3454 | `or membership.axis is None` | `->L3458` |  |
| 3455 | `or assignment.run_direction is None` | `->L3458` |  |
| 3456 | `or assignment.turn_direction is None` | `->L3458` |  |
| 3513 | `if ctx.exit_turns is None:` | `->L3514` |  |
| 3524 | `if family_id is not assignment.planned_family_id:` | `->L3525` |  |
| 3537 | `if (` | `->L3546` |  |
| 3555 | `run is None` | `->L3560` |  |
| 3556 | `or turn is None` | `->L3560` |  |
| 3557 | `or assignment.launch_coordinate is None` | `->L3560` |  |
| 3558 | `or assignment.minimum_runway is None` | `->L3560` |  |
| 3564 | `if segment_rank is None:` | `->L3565` |  |
| 3587 | `if planned_corner_offsets is None:` | `->L3588` |  |
| 3654 | `if source_turn_changed and settled is None:` | `->L3655` |  |
| 3684 | `if rank is None:` | `->L3685` |  |
| 3751 | `if current.keys() != snapshot.geometry.keys():` | `->L3752` |  |
| 3766 | `if current[key] != state:` | `->L3767` |  |
| 3799 | `station_id not in graph.stations` | `->L3802` |  |
| 3800 | `or lane.line_id not in graph.station_lines(station_id)` | `->L3802` |  |
| 3808 | `if (station_id, lane.line_id) not in station_offsets or (` | `->L3815` |  |
| 3823 | `if len(transition_routes) != 1:` | `->L3824` |  |
| 3861 | `route.exit_lane_transition_plan_id != str(exit_turn_plan.id)` | `->L3866` |  |
| 3862 | `or route.offset_regime is not OffsetRegime.BAKED` | `->L3866` |  |
| 3863 | `or route.points != expected_route.points` | `->L3866` |  |
| 3864 | `or route.curve_radii != expected_route.curve_radii` | `->L3866` |  |
| 3882 | `if not _fixed_axis_matches_plan(` | `->L3887` |  |
| 3893 | `if any(` | `->L3902` |  |
| 3910 | `if len(member_routes) != 1:` | `->L3911` |  |
| 3918 | `if route.exit_turn_family_id != assignment.planned_family_id.value:` | `->L3919` |  |
| 3925 | `if route.route_system_disposition == "compatibility":` | `->L3926` |  |
| 3928 | `if route.exit_turn_axis_id is not None:` | `->L3929` |  |
| 3937 | `if (` | `->L3951` | **defensive** -- The emission validation that raises when a rendered turn's axis, lead-in runway or direction differs from the assignment the route was bound to. The emitters derive all four from that same assignment, so the raise names an emitter ignoring its plan rather than a topology. |
| 3959 | `route.exit_turn_axis_id != str(assignment.axis_id)` | `->L3962` |  |
| 3960 | `or route.exit_turn_segment_rank is None` | `->L3962` |  |
| 3973 | `if (` | `->L3995` |  |
...
71/103 gates fully exercised.

Gates with an un-exercised arm:

| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 184 | `if run_direction is None or turn_direction is None:` | `->L185` |  |
| 186 | `if (run_direction in {Direction.R, Direction.L}) == (` | `->L189` |  |
| 198 | `if not candidate_ranks:` | `->L199` |  |
| 215 | `if ctx.exit_turns is None or not pending_plan_ids:` | `->L216` |  |
| 279 | `if source is None or source is route:` | `->L281` |  |
| 322 | `if route is None:` | `->L323` |  |
| 329 | `if route is None:` | `->L330` |  |
| 362 | `if ctx.exit_turns is not None:` | `->L364` |  |
| 364 | `if ctx.convergences is not None:` | `->L366` |  |
| 540 | `if route.exit_lane_transition_plan_id is not None:` | `->L541` |  |
| 542 | `if route.fan_plan_id is not None or route.fan_route_emitter is not None:` | `->L543` |  |
| 544 | `if convergence_owns_segment_boundary(route, segment_rank):` | `->L545` |  |
| 566 | `if band is not None:` | `->L567` | **needs-review** -- _channel_bounds' reserved-band arm. The bundles that reach the feasibility search carry no realised reservation band for the channel's segment, so bounds are always taken from the gap alone. Reachable, wants a fixture whose crowded member channel also sits in a reserved band. |
| 604 | `if bounds.band is not None and not bounds.band.lo <= candidate <= bounds.band.hi:` | `->L605` | **needs-review** -- _candidate_clears_runway's band rejection, unreachable while `if band is not None:::#1` never populates a band on this path. Reachable. |
| 664 | `if abs(channel.x - target) <= COORD_TOLERANCE:` | `->L666` | **needs-review** -- _align_same_line_channels' re-seat arm. Its one fixture (genomeassembly_organellar) still reaches the gate but now finds the channel already on its carrier's claim coordinate, so the reseat is skipped. Bisecting the range shows 0f58d60c -- ranking member channels over a gap's whole population -- is what makes the claim and the channel coincide on arrival, not the bundle-pitch packing. Reachable; the frozen render corpus means no fixture was authored. |
| 689 | `if coordinate is not None:` | `->L644`, `->L690` |  |
| 761 | `if overlap <= MIN_CORRIDOR_Y_OVERLAP:` | `->L762` |  |
| 855 | `if item.candidate.route.line_id in claim.line_ids and (` | `->L858` | **needs-review** -- feasible()'s same-line-carrier clause, which forbids a candidate delta that would split a carrier the bundle already shares with a claim. No fixture reaches the feasibility search with a same-line source-compatible claim in range: _align_same_line_channels seats such a member on its claim before the bundle allocation runs. Reachable, wants a fixture. |
| 858 | `if abs(coordinate - claim.coordinate) > COORD_TOLERANCE:` | `->L859`, `->L860` | **needs-review** -- The rejection inside feasible()'s same-line-carrier clause. Unreached for the same reason as its enclosing gate `if item.candidate.route.line_id in claim.line_ids and (:::#1`. Reachable. |
| 998 | `if radii is not None:` | `->L1003` |  |
| 999 | `if 0 <= rank - 1 < len(radii):` | `->L1001` |  |
| 1001 | `if rank < len(radii):` | `->L1003` |  |
| 1033 | `elif abs(start[0] - end[0]) <= COORD_TOLERANCE:` | `->L1023` |  |
| 1101 | `if movable_run:` | `->L1102` |  |
| 1146 | `if context is not None:` | `->L1148` | **defensive** -- _convergence_context_route returns None only when ctx.edge_by_key has no entry for the key, and edge_by_key is built from every graph.edge while the key comes from scaffold.edge_order, which is derived from the same edges. Same never-taken lookup as the sibling `if edge is None` guard on the non-convergence path (0 of 141 fixtures). |
| 1149 | `if key in ctx.skip_edges:` | `->L1150` | **candidate-dead** -- Subsumed by the convergence-edge check two lines above. Instrumenting all 348 fixtures: ctx.skip_edges holds exactly one merge-junction->entry-port hop, it is always present in scaffold.edge_order, and it is always also a convergence member edge, so the loop continues at the convergence arm and never tests skip_edges (0 of 141 fixtures). Proposed simplification: drop the two lines. Not applied here -- deletion is a deliberate separate pass, and a skip hop that is not convergence-owned is not ruled out by contract. |
| 1152 | `if edge is None:` | `->L1153` |  |
| 1156 | `if family_id is None:` | `->L1157` |  |
| 1178 | `if system_id not in failures:` | `->L1182` |  |
| 1339 | `or route.route_system_disposition != RouteSystemDisposition.PLANNED.value` | `->L1341` | **needs-review** -- validate_member_geometry_emission' skip for a route that holds a frozen member geometry plan while its route system did not end up planned. build_member_geometry_execution drops every candidate of a system that fails, so a plan-holding route can only be non-planned when the system is escalated after the freeze -- which happens exactly when a convergence conflict escalates it to the compatibility disposition. No corpus system carries such a conflict, so the operand never decides. Reachable through the same fixture that would close any of this module's conflict arms. |
| 1342 | `if tuple(route.route_system_owned_segment_ranks) != plan.owned_segment_ranks:` | `->L1343` |  |
| 1348 | `if actual != (channel.start, channel.end):` | `->L1349` |  |

## `normalize.py`

325/456 gates fully exercised.
