...
Safari does not, and this needed checking twice to get right. Playwright's
bundled WebKit gave inconsistent results across repeated runs (occasionally
dark, mostly light, with no clean rule), which looked like a race condition
rather than real browser behavior worth trusting on its own. Verified instead
against actual Safari on macOS - real system Dark Mode toggled via System
Events, a real `<img>` served over `http://`, the rendered pixel read back
through a same-origin canvas rather than eyeballed: 12/12 trials across every
combination of `<img>` `color-scheme` and real OS preference rendered
**light**. Safari's own top-level page correctly detected the real OS toggle
throughout (`matchMedia` flipped as expected), so the toggle genuinely
worked; the `<img>`-embedded SVG simply never responded to it, in either
direction. `light-dark()` is still being parsed - an unsupported function
would drop the whole declaration and fall back to SVG's own default fill
(black), not render the _first_ branch - so this reads as Safari not wiring
up the cross-document "used color-scheme" substitution at all, rather than a
partial or flaky implementation of it. Don't rely on this mechanism in
Safari in either direction, and don't trust Playwright's WebKit alone for
verifying browser-specific CSS edge cases like this one; where it's
practical, check the real browser.
...
files - but only document it as "adaptive in Chromium and Firefox, fixed to
its light variant in Safari," never as "adaptive everywhere." The Safari gap
is a real engine limitation with no authoring fix: no `.mmd` directive,
render flag, or CSS trick on the embedding page changes it. It also fails
safely, not badly - the image consistently shows its light palette rather
than breaking, corrupting, or flickering between states. That combination
(works on the two engines that matter most for most audiences, degrades
predictably on the third) is what makes one file a reasonable choice instead
of a broken promise - claiming full cross-browser parity from it would not be.
