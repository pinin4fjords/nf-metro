...
adaptation such a theme gets.

This is exactly the theme [nf-core/rnaseq](https://github.com/nf-core/rnaseq)
renders for its README map (`--theme light`, per its own
[`docs/dev/metro_map.md`](https://github.com/nf-core/rnaseq/blob/master/docs/dev/metro_map.md)),
embedded the ordinary way, as a committed SVG referenced by URL. Its checked-in
file confirms it: zero `light-dark(`, zero `var(--nfm`, no `color-scheme` on
the root - it predates chrome CSS, pinned to `nf-metro>=0.5.4`. Regenerating
it with a current release would add `var(--nfm-*, ...)` fallbacks (still
single-valued, `light` still has no pair to offer `light-dark()`), but the
map's actual light/dark behavior wouldn't change either way.
