...
2. Declare `color-scheme: light dark` on the root if the file needs to work
   standalone - opened directly, or dropped into a host with no opinion on
   theme.
3. Inlining it into a page you control? Remove that declaration instead, so
   the SVG inherits the page's `color-scheme` - redeclaring it overrides the
   host's choice rather than following it.
4. Referencing it via `<img>` instead (the safe way to embed content you
   don't fully trust, since it sandboxes scripts and external loads)? The
   host can steer it by setting `color-scheme` directly on the `<img>`
   element. This works in Chrome and Firefox regardless of what the SVG
   itself declares. Safari doesn't honor it - it always falls back to the
   viewer's own OS setting there instead of the host's, safely, just not
   host-controlled. That's the ceiling for a single file; no fifth step
   closes the Safari gap.
