...
Firefox honor this cleanly, independent of the OS preference either way.
WebKit only honors it partially: forcing the `<img>` to `light` works
regardless of the OS setting, but forcing it to `dark` only takes effect
when the OS _also_ prefers dark - otherwise WebKit falls back to light. A
host relying on this to pin `dark` against a light OS should not assume
WebKit support.
