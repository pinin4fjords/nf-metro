...
```html
<!-- isolated: never sees the host page's color-scheme, no matter how the
     SVG file itself was rendered -->
<img src="map.svg" />

<!-- in the page's own document: inherits color-scheme normally -->
<svg>...</svg>
```

The `<img>` tag loads the SVG into its own internal document, which never
inherits a host page's `color-scheme` - only the OS-level preference reaches
it, exactly as in standalone mode above. `<object>` and `<embed>` behave the
same way. That's the other reason (alongside `--no-self-color-scheme`)
nf-metro's docs site inlines its maps instead of pointing an `<img>` at them.
...
preference. To make it follow a manual toggle on your own site instead, drop
