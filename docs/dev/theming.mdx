...
preference either way. WebKit does not: across repeated runs it tracks the
OS preference rather than the `<img>` element regardless of which value is
declared, and does so inconsistently even then (a decode/paint race, not a
CSS rule) - the same instability shows up with no `color-scheme` declared on
the `<img>` at all. Don't rely on this mechanism in WebKit in either
direction.
...
files - but only document it as "adaptive in Chromium and Firefox, best
effort in Safari," never as "adaptive everywhere." The Safari gap is a real
engine limitation with no authoring fix: no `.mmd` directive, render flag, or
CSS trick on the embedding page changes it. It also fails safely, not badly -
the image just keeps showing whichever palette the OS/browser already
defaulted to, rather than breaking or looking corrupted. That combination
(works on the two engines that matter most for most audiences, degrades
gracefully on the third) is what makes one file a reasonable choice instead
