...
against the rules at emission, and lets the unique predicate that holds own the
route.
...
| 13  | `MERGE_TRUNK_AROUND_BELOW`            | `merge trunk around below`              | A primary merge trunk has no target-side channel.                                     | `_route_merge_trunk_around_below_feeder` loops under the target.                                             |
| 14  | `MERGE_TRUNK`                         | `merge trunk`                           | Merge classification names this feeder as the primary trunk.                          | `_route_merge_trunk_feeder` emits the U-shaped trunk.                                                        |
...
| 18  | `BYPASS_LEFT_ENTRY`                   | `bypass LEFT entry`                     | A bypass reaches the ordinary LEFT-entry wrap leaf.                                   | `_route_bypass_left_entry` emits the wrap.                                                                   |
| 19  | `BYPASS_LEFT_EXIT_AROUND_BELOW`       | `bypass LEFT exit around below`         | A bypassing LEFT exit must loop below a far-side LEFT entry.                          | `_route_bypass_left_exit_around_below` emits the loop.                                                       |
...
| 21  | `BYPASS_PACKED_CELL_SAME_ROW`         | `bypass packed-cell same row`           | A packed same-row sibling provides the handoff.                                       | `_route_bypass_packed_cell_same_row` shares the sibling corridor.                                            |
| 22  | `BYPASS_RIGHT_ENTRY_CROSS_ROW`        | `bypass RIGHT entry cross-row`          | A bypass reaches the cross-row RIGHT-entry leaf.                                      | `_route_bypass_right_entry_cross_row` emits the wrap.                                                        |
