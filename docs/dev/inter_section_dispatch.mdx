...
<Aside type="note">
If `assert_render_curve_invariants` fires, the fix is always to route the new shape through the centreline bundle builder — not to widen tolerances or add exceptions to the guard. A guard failure means a route was assembled outside the builder, bypassing the construction that keeps bundles correct by design.
</Aside>
