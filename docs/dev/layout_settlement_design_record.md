...
These corpus measurements and rejected alternatives informed the
envelope-settlement design. Each observation describes a particular commit. None
of them are layout invariants, and none state current corpus totals.
...
Every figure here is historical evidence. A named test identifies a
...
The settlement contract originally quoted 557 realized gap reservations and 1007
...
launch anchor, and the intersection reduction therefore had no observable
counterexample.
...
normalized by `measured_distance`.
...
`14.000000000000057`, and a bare ceiling therefore allocated 15px. The
resulting maps were 802px tall instead of 801px. The other 367 renders were
byte-identical.

Four fixtures did not support the origin comparison, because a uniform
...
requested less width after rerouting, and none requested more. The frozen
ledger therefore did not under-size a boundary drawn by that corpus. The current
...
`ConvergenceConflictKind` conditions. Three of those conditions cannot state a
case. `CHAINED_SAME_LINE` compared plans that
...
radius as the separation two lanes of one bundle owed each other. The corrected
arm asks each pair for the clearance its own kind requires. The count is
...
neutralizing each settlement pass in turn and recording every pair the condition
would then have caught.

With `_settle_shared_trunk_channels` neutralized, the same-line arm of
`SHARED_TRUNK_CHANNEL` catches 5 pairs on `merge_around_below_leftmost`, all of
them central-run pairs. Its co-traveling arm catches none anywhere, with or
...
With `_settle_opposing_landing_channels` neutralized, `SHARED_APPROACH_CHANNEL`
...
requirement. `MERGE_GAP_MIN = 50` holds that 9px of headroom, not fixture
geometry. Its three excuse predicates fire nowhere in the corpus.
...
rather than any of the totals listed earlier.
...
settlement kept the conflicts at 0px separation. Those results therefore came
from stranded junctions rather than from usable capacity.
...
carries that junction with the sections it joins. The arms therefore remain 0px
apart at every tested capacity. The gap is a missing shared-channel planning
decision, not a distance settlement can allocate.
...
a neighbor's caption ink or reserved band in their claimed strip, all caused by
...
exposed 27 failures among 356 rendering fixtures. Twenty-three were in the
own-section interior guard and 11 in the port-boundary guard, with overlap
...
`longread_variant_calling`. Settlement therefore takes their maximum and
translates once.
