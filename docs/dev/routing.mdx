...
decisions, immutable member templates and complete convergence decisions. It
then builds one atomic emission decision per route system, as described in
...
mode short-circuits that pipeline.
...
- When only some sections opt into rails (`has_rail_sections`), the edges
  ...
  the rest fall through to the normal handler chain.

Rail routing does not bundle. Each line runs along a single fixed horizontal
rail Y, assigned in `layout/rail_mode.py`. Each edge is therefore a straight
horizontal run at its line's rail Y, and shared stations render as interchange
pills bridging the rails.
...
2. `_route_tb_section` handles edges touching a `TB` section. It dispatches over
   ...
   (`_route_perp_entry`). Each shape describes a centerline and fans it through
   the bundle builder, `build_tapered_bundle` or `build_offset_bundle`. No
...
This fan-out exercises all four handler families at once. Inter-section
L-shapes connect the sections, TB-section routing applies inside the vertical
connector, entry runways compress the diagonal at each section's input port,
and intra-section diagonals handle the station-to-station edges within each
horizontal section.
...
materialized once before final convergence settlement. The general gap pass
therefore allocates only the channels that remain unowned. For a gap-allocated
planned turn, that pass validates the planned column or row and recomputes its
expected corner radius rather than choosing a new seat. A snapshot ratchet
checks source turns after every relevant pass.
...
Routing answers two kinds of question. Keeping them separate avoids recovering
authored intent from helper nodes that the resolver inserted.
...
- `compute_bundle_info` groups final edges that share a routing corridor. An
  authored `BundleRun` means only that connectors share exact authored
  endpoints.
...
A pass asking whether it may move a coordinate reads one of two predicates in
...
- `route_system_owns_segment_boundary(route, rank)` asks whether a convergence or
  member plan owns a corner at either end of the segment, which is what a pass
...
This debug overlay makes the offset geometry visible, showing each line's
...
reservation realizes, measured against the blockers over the corridor's own
...
just widened these boundaries. The pass that publishes the ledger is therefore
...
corridor takes. Every claim crossing one boundary realizes the same band.
Several independently placed corridors can therefore each be put in it without
any of them seeing the others. Two can then settle less than one `OFFSET_STEP`
apart, close enough that two distinct lines paint a single two-tone stripe and
one of them cannot be read at all. `_separate_fused_cotravelling_runs` is the
closing pass that sees every corridor at once and restores the step. The unit it
moves is a _track_, meaning every run of one line drawn on one lane through one
corridor. Re-seating therefore cannot split a fused fan-out into two parallel
same-color runs. `check_no_fused_cotravelling_lines` is that pass's
postcondition on the render chokepoint.

Plans seat distinct co-traveling trunks against frozen member corridors before
ownership begins. The final check includes immutable tracks and attributes a
violation to its route system and exact owning plans.
...
authored connectors, emission member, plan IDs and reservation IDs. A post-pass
therefore cannot silently detach geometry from its owner.
...
the exact geometry the renderer is about to draw. A defective route aborts the
render with a message naming the offending edge, rather than shipping a broken
diagram. The check is always on, independent of `compute_layout`'s `validate`
flag.
...
- `check_no_hanging_routes` is the general backstop. **Every** route's two
  ...
  stub terminates on its rail. This generalizes the merge-only check to any
