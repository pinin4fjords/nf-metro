...
decisions, and complete convergence decisions described in
[route planning and observation](/nf-metro/dev/route_plan/). Inter-section
handlers consume those decisions during routing.
...
complete exit group. The convergence planner then freezes the target-side
trunk, landings, continuation, and endpoint owners for every complete route
system. Either planner commits its complete ownership unit or leaves that unit
on legacy routing. The dispatcher then tries a fixed sequence of handler
functions. **The first handler that returns a `RoutedPath` wins**; handlers that
do not apply return `None`.
...
and convergence trunks and joins owned by the planners are immutable during
these passes. A snapshot ratchet checks source turns after every relevant pass.
...
convergence planner combines that identity with settled coordinates and the
canonical templates once, before emission. `_classify_merge_edges` supplies
...
An ordinary horizontal channel between stacked rows derives its Y band from
the bottom edge of the upper row and the header edge of the lower row. Those
whole-row edges can name sections outside the route's span, so they are only a
proxy for the corridor's real blockers.

Envelope settlement supplies the final reroute with the immutable reservation
ledger it consumed. `build_reserved_row_bands` re-measures each row-gap claim
against settled geometry, including the recorded row and column translations,
and intersects the clear spans claimed at each boundary. Routing helpers place
their channel inside that realised band. Routes at unclaimed boundaries keep
the ordinary row-edge derivation.

Only the reroute after a real envelope translation receives reserved bands.
The first routing pass remains the observer that publishes the ledger, and a
render with no settlement work does not route again.
