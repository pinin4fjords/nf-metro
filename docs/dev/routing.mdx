...
The normal dispatcher path builds the semantic scaffold and complete exit-turn
decisions described in [route planning and observation](/nf-metro/dev/route_plan/).
Inter-section handlers consume those decisions during routing.
...
complete exit group and either commits the whole group or leaves the whole
group on legacy routing. The dispatcher then tries a fixed sequence of handler
...
owned by the planner are immutable during these passes. A snapshot ratchet
checks them after every relevant pass, and the final invariant checks each
assignment, lane offset, route family, turn direction, and turn axis.
...
| Which corridor is clear, which feeder is the merge trunk, or where did a route actually turn? | The laid-out graph and routed polylines    |
...
The query selects the semantic candidates for fan and merge handling. Geometry
then makes the routing decision. For example, topology identifies a convergence
junction and its entry port, while `_classify_merge_edges` still reads final
coordinates to choose the farthest bypass feeder as the trunk. The same rule
applies to offsets, normalization, and invariants: topology scopes the group;
the final graph or polyline supplies the measurements.
...
- exit-port alignment follows immediate resolved successors because a branch
  ...
  destination;
- normalization groups settled vertical channels, trunks, and route endpoints;
...
`COORD_TOLERANCE` limit in Tier C. `_land_merge_feeders_on_trunk` enforces this
limit after the routing passes have placed the feeder and trunk.
