...
`observe_route_edges` can accompany that production run with an immutable
[route plan observation](/nf-metro/dev/route_plan/). The plan records semantic
route systems, the handler family that actually emitted each inter-section
member, explicit merge coverage, and settled layout provenance. It is an
observer only: normal routing does not build it, and routing never reads it.
