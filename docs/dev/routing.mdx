...
Both allow `2 * CURVE_RADIUS` of slack, since they report a path stranded in
open space. A terminus only a few pixels off still draws a stub, so the
precision property -- a merge feeder ends **on** the trunk's centreline, at or
before the trunk's own corner -- is asserted separately by the Tier-C oracle
`check_merge_feeders_land_on_trunk` at `COORD_TOLERANCE`. It is
`_land_merge_feeders_on_trunk` that establishes it, reading the trunk's settled
channel after every pass that could move either leg.
