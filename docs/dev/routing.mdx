...
settlement, validation, or observation.
...
`TOP_ENTRY_L_SHAPE`, `LEFT_ENTRY_WRAP`, `RIGHT_ENTRY_WRAP`, `TB_BOTTOM_EXIT`)
and a `TurnSequence` - the ordered list of `Corner`s (each an
`(incoming, outgoing)` `Direction` pair).
...
which also sanity-checks that every key is well-formed and that the table stays
non-empty. Same-row L-shapes and the straight TB `(BOTTOM, TOP, 1, 0)` drop are
intentionally absent because dedicated classified families cover them.
