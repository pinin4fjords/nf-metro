...
reports what one exact set of layout commitments produces. It does not generate
candidates, rank them, choose one, or retry a normal render.

The normal CLI and Python rendering APIs never call the executor.

The executor module is not shipped in the nf-metro wheel. Drive it from a
source checkout.
...
candidate cannot replace an authored or caller-owned value. The parser rejects
duplicate, contradictory, malformed, partial, and unknown commitments before
layout.
...
again, which catches a commitment that inference accepted but failed to honor.
...
6. Render planning observes the final centered routing invocation used by the
   plan.
...
The result keeps every completed piece of evidence. An SVG emission failure
...
visual damage. Because `PermissiveGuardWarning` inherits from that category, a
...
baseline always runs first. The coordinator reports candidates that
cannot start because of a limit explicitly as unattempted rather than as
timeouts.

Workers send versioned, sequenced frames over a one-way pipe and split large
evidence into small atomic frames. A receive therefore cannot block
indefinitely after only part of a message arrives. The parent receives and
verifies the complete payload before joining a successful worker. It joins
before reading the exit code, and it terminates or kills and reaps every
timed-out worker.
