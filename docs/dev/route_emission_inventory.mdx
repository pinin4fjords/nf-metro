...
Route emission covers every production stage that can create, move, suppress,
validate, or observe routed geometry. The boundary is `_route_edges`. Section
...
- a member-geometry plan builds and freezes the classified family's complete
  production path

Production emission consumes that decision. It does not run an ordered handler
chain, retry another family, or reconstruct a legacy route. The planned family
classification is therefore the single inter-section dispatch surface.
...
planner settles that frame before freezing it. Post-emission normalization
therefore cannot recreate the former compatibility route by moving the copied
path.
...
convergent descents, destination-tail bundles, and corridor-clearance bands all
settle before the owning plan freezes them.

Destination-tail settlement returns the exact horizontal segment identities it
owns. Corridor holding treats only those segments as fixed. Perpendicular
channels therefore remain free to settle. That keeps ownership narrow and avoids
freezing an entire route because one tail band is planned.
...
Planner reason registries remain as diagnostics and as a fail-closed boundary
for hand-built or unsupported inputs. They do not select a production emitter.
On the routable corpus, every system reaches emission as `PLANNED`, and its
named planner owns every diagnostic reason.
...
[Inter-section dispatch](/nf-metro/dev/inter_section_dispatch/) documents the
family table, and
[routing gate coverage](/nf-metro/dev/routing_gate_coverage/) publishes the
gate-arm coverage.
