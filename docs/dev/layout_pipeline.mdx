...
- **Stage 1.3**: Place sections on the canvas grid by topological
  layering of the section DAG. When a stacked LEFT-to-LEFT half-turn feeds a
  split, reflect the consumer's branch tracks so its in-section order matches
  the reversed bundle delivered at the entry.
