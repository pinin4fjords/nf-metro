...
and builds complete exit-turn plans before emitting the first inter-section
path. Whole-graph rail mode takes its documented short circuit instead.

`compute_station_offsets` first produces the ordinary offset map. Immediately
before dispatch, `_route_edges` builds the exit-turn plan once. The planner
compacts each supported group onto its active lanes, commits the offsets it
owns across the source seam, and assigns source-side turn axes from that same
order. Ordinary and observed routing share this boundary. An always-on
invariant rejects any downstream route-mutating pass that changes a committed
lane or axis.
