...
The layout pipeline turns a parsed metro graph into placed coordinates. Start
here to hunt a layout bug, fix a visual regression or add a new transformation
pass.
...
Achieving all of this in one pass is intractable. Some constraints are local,
such as each section's internal layout, and some are global, such as trunk Y
alignment across an entire row. The pipeline instead chains many small passes,
each of which mutates the graph and preserves the invariants of the passes
before it.
...
When reading the stage walkthrough, keep the two kinds apart. A structural phase
that looks as though it moved content is really moving an anchor and letting
content follow, and a content phase that looks order-dependent is, by
...
Before Stage 1, the engine recognizes complete authored fans and diamonds from
connector identity and resolver lineage. Each supported fan gets one immutable
relative plan covering its branches, opening and landing order, centerline,
...
branches. The legacy disposition carries a deterministic diagnostic. Unsupported
cases therefore stay visible.
...
where ports become positioned. They also align with the Pass A, Pass B and
...
- **Stage 1.3**: Place sections on the canvas grid by topological
  layering of the section DAG. When a stacked same-side half-turn feeds a
  split, on either side, reflect the consumer's branch tracks so its in-section
  order matches the reversed bundle delivered at the entry.
...
- **Stage 3.2**: Align LEFT / RIGHT entry ports to the incoming source
  Y so the inter-section horizontal run is straight, and align TOP / BOTTOM
...
- **Stage 3.4**: Align LEFT and RIGHT exit ports on row-spanning fold sections
  ...
  its row-mates'. The move therefore finishes by top-aligning the rows it
  disturbed, meaning the contiguous column groups that share their bbox tops.
- **Stage 3.5**: Grow an LR or RL section's left and right bbox edges so its
  ...
  neighbor, which is why the stage re-enforces the inter-column gaps.
- **Stage 3.6**: Level the bbox left edges of column mates that start their
  ...
  the same runway. A grid row's sections share a trunk Y, and their tops
  therefore always frame the same thing. A grid column's sections do not share a
  trunk X. Leveling boxes whose content starts at different X therefore buys an
  aligned edge at the price of an empty band. Stations and the right edge stay
  put, while LEFT ports ride the edge out to the column line. A box stops short
  where a left neighbor in its own row band needs the inter-column corridor.
...
Pull ports toward downstream stations to remove unnecessary detours. Consolidate
the inter-section trunk Y across each row, then redistribute fan-out and
full-bundle columns around the trunk.
...
- **Stage 4.5**: Keep ports at least `y_spacing` from terminus stations so file
  icons do not overlap routed lines. This may expand bboxes.
- **Stages 4.6 to 4.7**: Recompute grid-group bboxes, then re-run row
...
- **Stages 4.9 to 4.10**: Redistribute fan-out siblings and full-bundle columns
  around the trunk. Stage 4.9 first materializes complete semantic fan plans,
...
- **Stage 5.1**: Position every junction station in the inter-section
  gap. Fan-out junctions sit at the exit port's Y, and merge junctions sit
  near the entry port.
...
- **Stage 5.5**: Snap inter-section LR and RL port pairs back to a shared Y,
  because the compaction in Stage 5.4 may have drifted them, then re-position
  junctions to follow.

### Stage 6 - Pass C: vertical settling & finishing

The long settle. Its sub-stages clean up the consequences of Stages 1 through 5,
snap everything to the grid, then restore the invariants each cleanup pass
breaks. The remaining geometric details follow: loop-side X recenter, bbox
shrink and grow, canvas snap and port re-align.
...
- **Stage 6.15a**: Fit bbox tops to content, symmetric with the bottom shrink in
  ...
  was sized for (#406). It also shrinks an empty band that the transient row-top
...
- **Stage 6.17**: Re-materialize complete semantic fan frames on their settled
  centerlines without resizing their sections or row mates. Two-way plans under
...
- **Stage 6.18**: Expand a half-pitch branch to a full row when its mirrored
  partner has gone, which keeps an isolated branch from sitting between grid
  rows.
...
- **Stage** (1-6) groups by **what kind of mutation** the pass performs: section
  construction, globalization, port positioning, port refinement, junctions and
...
- **Pass** (A, B or C) groups by **how much of the layout is final** when the
  ...
  refines ports on a fixed station layout. Pass C operates on finalized stations
  and ports.
...
- **A station moved when it should not have.** Work out which stage's
...
- **A guard fired with `after final: ...`.** The invariant holds only at the end,
  which puts the regression anywhere in Pass C. Run with `validate=True` and use
  the per-checkpoint bisection in `_run_pass_c_guards` to localize it.
- **A new fixture lays out badly.** Render it with `nf-metro render`, then
  inspect the SVG against the stage descriptions earlier on this page to work out
  which stage handles the problem area. Read that sub-stage's entry in
  CONTRACT.md.

## Why so many sub-stages

The Pass C tail, Stages 6.1 to 6.18a, looks excessive. Each sub-stage exists
because:

1. A real-world fixture exposed a bug.
2. A targeted helper fixed it.
3. The helper sits at the point in the pipeline where it has the
   inputs it needs and does not disrupt earlier-established
   invariants.
...
`Phase 13`, `13a`, `13d2`, `13h.1` and `13k2`, which grew a suffix each time a
sub-stage was inserted between two existing ones. The new scheme keeps the same
ordering but makes the sequence walkable. The historical context lives in the
git log and in the "Adding a new stage" section of CONTRACT.md.
