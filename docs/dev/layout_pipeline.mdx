...
Stages 4.9 and 6.17 materialise the plan against a read-only centreline captured
after the structural anchors settle at each boundary. The placement phase never
derives its frame from a plan-owned station that it is about to move.
