...
runways, offset carriers, and any dedicated route emissions. Stages 4.9 and
6.17 materialise that plan against the anchors available at those boundaries.
The authored straight or symmetric appearance is frozen separately from the
structural question of whether the branches reconverge.
...
exit-turn plans before emitting the first inter-section path. Whole-graph rail
mode takes its documented short circuit instead.
...
exit-turn plan once. The exit-turn planner compacts each supported group onto
its active lanes, commits the offsets it owns across the source seam, and
assigns source-side turn axes from that same order. Ordinary and observed
routing share this boundary. Always-on checks reject changes to a committed
lane or axis and ensure every dedicated fan emission is consumed exactly once.
