...
- **Stage 3.6**: Level the bbox left edges of each grid column's
  contiguous row run by growing the narrower boxes leftward, the X mirror
  of the row top-align at Stage 5.3. Stations and the right edge stay
  put; LEFT ports ride the edge out to the column line. A box stops short
  where a left neighbour in its own row band needs the inter-column
  corridor, or where a LEFT port sits on a line channel shared with two
