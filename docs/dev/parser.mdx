...
Switching to `lalr` would turn any partly-matched unrecognised line into a fatal parse error instead of a dropped `junk` statement. The leniency policy (warn, don't crash) depends on Earley's ability to backtrack. The parse runs once per render, so Earley's extra cost over `lalr` is not worth trading away for this behaviour change.
</Aside>
