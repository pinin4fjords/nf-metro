...
The parser turns `.mmd` text into a `MetroGraph`. Start here to add a node
shape, a `%%metro` directive or a new statement form, or to understand the
grammar.
...
1. **Front-end.** Recognize each line's shape, whether it is a node, an edge or
   a directive, and pull out its pieces. The grammar recognizes statement
   _shapes and boundaries_. Python handles directive _payloads_ and graph
...
recognizing each line: check for a `graph` header, then for a `subgraph`, then
for an arrow, then try six node-shape patterns in a fixed order. That works,
...
A **grammar** works the other way round. Instead of the recognizing
instructions, you write down _the rules of what a valid line is_ and a library
does the recognizing. nf-metro uses
...
sequences. A source shape like `[X]` therefore stops at the arrow rather than
swallowing it.
...
2. **Transform.** `_StatementTransformer` walks that tree and flattens it into
   ...
   `_Edge`, `_End`, `_Comment` and `_Junk`. The transformer normalizes as it
   goes. Each statement carries structured fields rather than raw tokens.
...
choices, both possible fold-threshold inputs, and line-order inputs. The
line-order record distinguishes a directive, a caller override, and the default,
and also keeps line definition order. The effective decision records the chosen
policy and source. A caller override is captured before inference but remains
applied at the established API override stage, preserving current layout
behavior. Because the snapshot is immutable, later inference cannot make an
...
id and declares any inline-shaped endpoints, and a `_Directive` is routed to a
handler.

The driver is an in-order loop by design. Source order matters because the
dictionary insertion order of stations affects downstream layout. The grammar
...
handler functions, because a grammar cannot express behavior like "warn about
this and ignore it", which several directives need. The grammar delivers the
...
planes. The bespoke handlers in the table carry grammar, meaning fields,
sections and coordinates, that the generic registry cannot express.
...
`exit_hints`, each a `(side, [line_ids])` list. Real inter-section edges drive
port creation later.

## Leniency and error policy

The split is deliberate. The **grammar and parse layer is lenient** about syntax
it does not recognize and warns rather than crashing. **Semantic validity is a
separate, stricter phase**. A typo in a node line should not abort a render of
an otherwise-fine diagram, but an edge that names no metro line is a modeling
error.
...
The unrecognized-line case is handled in the grammar by a low-priority
catch-all:
...
`JUNK` matches any line, but its negative priority means it wins only when
...
fallback. A committing `lalr` parser cannot backtrack a partly-matched line and
would turn it into a fatal error instead.

<Aside type="danger" title="Do not switch the parser to lalr">
  Switching to `lalr` would turn any partly-matched unrecognized line into a
  ...
  parse runs once per render, and Earley's extra cost over `lalr` is small next
  to the leniency it buys.
</Aside>
...
7. Build the immutable `RouteTopology` described in
   [Authored route topology](#authored-route-topology).
...
`RouteTopology` captures route meaning before the resolver replaces authored
cross-section connectors with port and junction chains. Authored facts are
captured before interchange expansion. A parser-local sidecar follows each
authored connector through interchange and terminus-convergence rewrites, but
...
ordering. The same input therefore produces the same topology under every hash
seed.
...
several when one exit leg needs parallel bypass helpers. Because the records use
scalar ids and tuples, duplicate authored connectors can share paths. Bypass
...
- `authored` is the frozen pre-inference snapshot described in
  [From text to `MetroGraph`](#from-text-to-metrograph).
...
keyed by the semantic `ConnectorId` plus `entry` or `exit`. Two lines sharing
one visible port can therefore retain different histories. If resolution changes
an authored side, the effective decision records the new side and retains the
...
- **A `%%metro` directive.** Write a `(value, graph) -> None` handler and add an
  ...
  specific message, and return rather than failing silently, per the
  [leniency policy](#leniency-and-error-policy). A directive that names a
  station, section or line id must buffer it and add a case to
  `_directive_references`. The id then resolves against the whole file rather
  than only the text before the directive.
- **A new statement form.** Add a rule and its terminal to `_GRAMMAR`, a typed
  statement dataclass in the `_Statement` union, a method to
  `_StatementTransformer` returning that dataclass, and an `isinstance` branch
  to the driver loop in `parse_metro_mermaid`.

## Equivalence checks

The grammar must produce exactly the `MetroGraph` the input implies. Changes
here are therefore checked by **comparing parsed model objects** rather than
rendered SVGs. The check parses every fixture in `tests/fixtures/`, `examples/`
and `examples/topologies/` and asserts that the model matches. Identical models
render identically, which keeps the gallery byte-identical without chasing
sub-pixel render drift. The grammar's coverage and behavior are pinned by
