...
4. Build the immutable `RouteTopology` observation described below.
5. `_insert_terminus_convergence_stations`.
6. `_resolve_sections` - the core rewrite (below).
7. `_insert_bypass_stations`.
...
cross-section connectors with synthetic chains. It is built immediately after
`infer_section_layout`, when section grid positions and inferred port hints are
available, and before terminus-convergence, port, junction, or bypass insertion.
Resolved entry and exit sides are calculated on an isolated copy; construction
does not mutate the graph that the resolver consumes.
...
divergences, and resolver-shaped convergences. Every connector links back to a
deterministic authored-edge ordinal and the line ordinal within that authored
edge. This distinguishes duplicate endpoint/line edges and comma-separated
multi-line edges without relying on source line numbers.
...
The resolver remains authoritative: later parser rewrites do not consume these
records yet, and topology construction must leave resolved graph and render
