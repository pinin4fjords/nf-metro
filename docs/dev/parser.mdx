...
so the ids a directive references cannot be resolved in its handler. Handlers
check only payload shape and buffer the ids; `_warn_unresolved_references` and
`_resolve_legend_combos` resolve them in `_finalize_graph`, after the whole
source has been applied and **before** layout inference, which synthesises
stations and fills `grid_overrides` for every auto-placed section. A handler
that resolved an id itself would report a false unknown for a forward
reference, and an audit placed after inference would stop seeing which ids the
