...
changes. All thirteen of the corpus's compatibility systems do not change at any
capacity up to 4464px, which is what proves their limit is a channel decision
rather than envelope allocation.

Thirteen is the live population; `COMPATIBILITY_CORPUS` carries fourteen rows.
The fourteenth is a control: a fixture the planner came to own keeps its row,
asserting that it publishes no compatibility system at all, so a system slipping
back onto that path fails a test rather than passing unnoticed.

That fourteenth is also where the decision below comes from.
