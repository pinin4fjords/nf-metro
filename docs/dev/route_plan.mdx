...
The row sweep and then the column sweep each visit every adjacent-index
boundary once in ascending order.
...
interfere, no separation ever decreases, and settlement is finite in the
number of boundaries rather than iterating toward a fixpoint. Both sweeps
consume one immutable ledger observed before any translation.
...
instead. `assert_reservations_are_settled` reports those as attributed
warnings and fails the strict path only for a deficit with no such
attribution, which would mean a corridor settlement was free to widen and did
not. The one final reroute receives the realised row-gap bands from that input
ledger. It places claimed inter-row channels inside the space their own
blockers leave, rather than re-deriving the channel from unrelated whole-row
edges. The final `RoutePlan` is observed from those routed coordinates.

The closing guard checks the two ledgers for different purposes. It projects
the immutable input claims through the recorded translations and proves that
settlement delivered their required capacity. It reads the final routed claims
without projection and rejects any new capacity deficit. Reserved-band routing
places the channels it owns inside their realised side clearances by
construction. No demand emitted by the final reroute can cause another
settlement sweep.
