...
4. Which source-side turn geometry was committed before emission?
...
a complete exit group. Emission bindings and corridor reservations describe
what production routing finally emitted.
...
`observe_route_edges` run the same exit-turn planner and consume the same
decisions. Ordinary routing returns only the routes. It does not construct a
...
| [#1657](https://github.com/seqeralabs/nf-metro/issues/1657) | Planned     | Convergence trunk extent, feeder landing sequence, and approach space                   |
