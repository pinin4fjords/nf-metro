...
Settlement declines a deficit whose far side is bounded by a section spanning
across the boundary rather than sitting beyond it -- typically a row-spanning
sidebar. That section is measured on the near side too, so its own box lies
inside the width the claim asks for and the claim describes no gap at all;
settlement publishes a `SettlementObstruction` naming it, and the author pin
behind it where the grid is authored, instead of translating rows that could
not help. That is the only deficit settlement can leave behind, because a
far-side blocker it cannot translate is exactly a spanning one.
`assert_reservations_are_settled` fails the strict path for every other
surviving deficit -- one settlement was handed and did not meet, and one the
settled re-route publishes -- since a route drawn through a clearance it does
not have is wrong either way. A claim with no allocatable gap is reported as a
ledger coherence problem rather than failing a map.
