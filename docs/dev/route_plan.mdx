...
Final envelope settlement re-evaluates the convergence systems previously
limited by opposing openings, shared channels, unowned members, or chained
same-line trunks. Once their claimed boundary space is available, every member
is planned with the same semantic membership, references, demands, and
ownership. The router materialises an allocated coordinate before semantic
plan validation instead of selecting a fallback band from incidental remaining
space. [#1658](https://github.com/seqeralabs/nf-metro/issues/1658) retains
ownership of emitter consolidation and retirement of displaced reconstruction
logic.

Two compatibility boundaries remain with explicit evidence and ownership.
Authored pins can prevent a claimed boundary translation; the limitation
records the reservations, blockers, pin decisions, and owner #1658, while a
strict render rejects it. Unresolved overlapping fan ownership can leave a
dynamic merge-entry member outside the convergence emitter; that whole system
records owner #1658 instead of treating additional envelope space as proof.
The latter covers the experimental `funcprofiler_upstream` topology without
extending settlement into fan/emitter consolidation.
