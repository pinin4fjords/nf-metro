...
flowchart TD
    ...
    P["route-system plans<br/>ExitTurnPlan, FanPlan, convergence plan"]
...
translation owner. All 144 realised canvas reservations now meet their required
width, and `assert_canvas_corridors_hold_their_claims` gates both the canvas-edge
margin and total capacity. It does not yet gate the _content_ side, because the
clearance charged there is longitudinally blind and assumes a caption above the
box; #1693 owns tightening that claim to the badge actually drawn, and the
shortfalls are published as attributed `reservation-deficit` diagnostics until it
does.
...
evidence that actually settles the question is `layout/capacity_probe.py`, which
asks the counterfactual directly: it grants a boundary generous capacity on a
deep copy of the settled geometry, re-plans, and reports whether the disposition
changes. All twelve of the corpus's compatibility systems do not change at any
capacity up to 4464px, which is what proves their limit is a channel decision
rather than envelope allocation.

Twelve is the live population, carried by twelve fixtures and collapsing to nine
distinct system-id strings because sibling fixtures share connector names;
`COMPATIBILITY_CORPUS` carries fourteen rows. The other two are controls: a
fixture the planner came to own keeps its row, asserting that it publishes no
compatibility system at all, so a system slipping back onto that path fails a
test rather than passing unnoticed.

One of those controls is where the decision below comes from.
`merge_around_below_leftmost` did change, at 22.5px, so what held it was
capacity, and the answer was not to buy it. Its route system converges twice,
and each convergence read its trunk
coordinate off a trial route taken with no knowledge of its sibling, so both
proposed the same lane of one inter-row channel and the planner declined the
