...
owner: 22 of 397 realised canvas reservations carry a capacity deficit, and a
`CanvasRegion` carries none of the row/column locality its gap-region siblings
do, so claims from unrelated parts of a map group into one reservation and the
width that follows describes no drawn corridor. Those are reported, not gated.
And the router's narrow-band fallback, which biases a run against the header
badge it cannot clear, fires only at gaps the ledger never claimed, so there is
no reservation to consume there.
...
No deficit is left behind for that reason, so none is exempt:
