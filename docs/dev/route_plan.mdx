...
case. `CHAINED_SAME_LINE` fired when two same-line trunks sat further apart than
...
decision. The surviving condition is `NO_APPROACH_SETTLEMENT_ROOM`, from
`_landing_trunk_flank_conflict`: a landing whose approach and trunk flank leave
no room between them, published with the axis, both run coordinates, and the
distance measured. Historical population and grant totals from the measurement
programme are recorded in the
[envelope settlement design record](/nf-metro/dev/layout_settlement_design_record/).
