...
<Steps>
  ...
  3. If you added behavior, add a topology fixture that covers it (see [Add a
  topology test](#add-a-topology-test)). 4. If you fixed a bug that had an
  `xfail` marker, remove the marker so the fix is locked in.
</Steps>
...
<Steps>
  1. Write a minimal `.mmd` that exercises the topology. 2. Save it in
...
Automated geometry checks verify that the coordinates are correct. They cannot verify that the result _looks_ right. Review layout and rendering changes by eye.
