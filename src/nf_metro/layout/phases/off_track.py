...
def _bump_off_track_clear_of_trunks(
    ...
    for sid in section.station_ids:
        ...
        # only the latter; a same-row station is therefore always a contesting
        # band.
        delta = getattr(st2, flow_axis) - off_flow
        same_row = abs(delta) <= SAME_COORD_TOLERANCE
        if not same_row:
            if flow_sign * delta <= SAME_COORD_TOLERANCE:
                continue
