...
def _fanout_route_maps(
    ...
    round a departure.  A back-extended start would read as an along-trunk
    gap the apex check falsely flags, so the downstream nearest the
    junction wins.
    """
...
def check_fanout_tail_join(
    ...
    for (jid, line_id), up in upstream.items():
        ...
        # starts BEHIND, its lead-in back-extended over the incoming trunk:
